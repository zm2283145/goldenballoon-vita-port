...
Every context exposes one prominent **Review all motion states** action. Select
runs idle, hover, and confirm; each vehicle runs both steering extremes,
reverse, boost, item, damage, spin, airborne, landing, and both finishes inside
one exact one-player game session and returns without manual navigation. Every
...
approval requires the current context battery bound to source, fit, LOD policy,
presentation settings, and the semantic-review contract. Detailed rows are
...
1. Donor-reference occlusion, named attachment attribution, representative
   course/vehicle conditions beyond the fixed semantic Workshop batteries, and
...
Select and vehicle fit approval use the source-and-fit-bound v4 semantic-review
contract and require the current three-state select or eleven-sample race
battery bound to source, fit, LOD policy, presentation, and review contract.
Both require
...
Named-draft v15 and Fit-history v6 preserve the new meaning. Every older
approval reopens once rather than silently upgrading single-pose select or
five-state vehicle evidence into complete semantic review.
