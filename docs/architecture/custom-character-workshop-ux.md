...
| Animation semantics | Executable sampling/diagnostics plus source-v4 compiled role contract, inference provenance, confidence, review lock, native hierarchy validation, reviewed engine-reference fallback motion, and bounded idempotent vehicle contact solving | Add visual role-map editor, richer reference clips, target overlays and joint-limit inspection |
...
| Vehicle support | Source-backed compatibility revision, runtime enable subset, independent transforms, and persisted per-vehicle hand/foot target offsets | Add visual target manipulators, contact-error feedback and complete vehicle test matrix |
