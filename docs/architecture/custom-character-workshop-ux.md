...
2. Optional author-defined joint constraint profiles and deterministic
   secondary motion remain. Animation Studio now has rendered 200% keyboard
   held-mode and virtual-controller transition-mode traversal through the real
   SDL/ImGui focus path, including responsive player/mode groups, spoken current
   values, and disabled-capture exclusion. Observed controller/screen-reader
   review and the remaining freeform editor gestures still need human
   qualification. The 13-state bounded reference library is complete;
   package-specific artistic review remains intentionally mandatory.
...
Optional authored joint constraint profiles, deterministic secondary motion, and
an explicit pole witness only if the solver gains an authored pole remain. The
...
Named-draft v18 and Fit-history v9 preserve the new meaning. Every older
