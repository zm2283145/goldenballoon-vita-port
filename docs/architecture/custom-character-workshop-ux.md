...
1. Donor-reference occlusion, named attachment attribution, representative
   course/vehicle/motion conditions, and unusual-proportion fixtures still
...
idempotent CCD vehicle contacts are implemented. Richer reference animation,
side-by-side transition review, spatial vehicle target/pole overlays, and
anatomical limit profiles remain.
...
Per-context fit approval is now a source-and-fit-bound v2 contract. It requires
...
Named-draft v13 and Fit-history v4 reopen older approvals instead of silently
upgrading their meaning.
