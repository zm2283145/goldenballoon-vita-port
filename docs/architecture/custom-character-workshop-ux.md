...
| Accessibility | Dedicated panel/tab/library/import/assignment controls carry spoken names, textual status, keyboard/controller navigation, and a narrow single-column fallback. The exact-test matrix and baseline/destructive controls have a rendered 200% keyboard/speech lifecycle gate. | Qualify every remaining editor tool with controller/screen reader and close spatial-tool gaps |
