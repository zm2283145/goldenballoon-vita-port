...
Separately attached wheels and props, opaque/transparent depth visibility,
unsampled geometry, and motion between sampled poses remain explicit
composed-scene review items.

Vehicle contexts also expose one prominent **Review representative motion**
action. It runs a fixed one-player battery—race start, full steer, airborne,
landing, and finish win—inside one exact game session and returns without manual
navigation. Every row waits for an engine-owned settled-pose generation and a
further sixty replacement draws so the state remains human-visible before
requesting fresh surface and asynchronous
opaque-depth witnesses. The summary reports all five rows, names only real
framing/body problems, calls out unqualified volume or blended-material cases as
visual checks, and identifies minimum qualified visibility and maximum contact
error. A fully invisible qualified opaque state blocks approval; unusual anatomy
and intentional overlap stay reviewable through an explicit acknowledgement.
Vehicle approval requires a current battery bound to source, fit, LOD policy,
presentation settings, and the motion-review contract. Detailed rows are kept
for the launcher session; the durable approval signature preserves the reviewed
decision and asks for a rerun when detailed measurements are wanted later.
...
| Animation semantics | Executable sampling/diagnostics plus source-v4 compiled role contract, inference provenance/confidence, transactional 16-role skin-joint editor with bind-pose hierarchy canvas, rest/bend controls and automatic review invalidation, native hierarchy validation, reviewed engine-reference fallback motion, bounded idempotent vehicle contact solving, and exact post-solve chain-root/bend/target/end witnesses. Animation intent enumerates all 13 engine states, detects active static-looking mappings, and durably disables/restores a mapping without deleting its source clip; runtime, readiness, history and v13 named drafts use the same decision. Animation Studio adds exact 0/50/100% held samples, digest-bound comparison captures, and repeating A/B transitions through the ordinary pose player with measured per-leg blend and motion-source evidence. | Add richer reference clips and joint-limit inspection; add an explicit pole witness only if the solver gains an authored pole rather than inventing one in UI |
...
1. Donor-reference occlusion, named attachment attribution, representative
   course/vehicle conditions beyond the fixed five-state Workshop battery, and
   unusual-proportion fixtures still
   need qualified evidence contracts. Exact final-scene opaque-depth visibility
   is now a nonblocking 8 x 8 boolean-region comparison against an isolated
   replay; it observes attached depth writers collectively, accepts a completed
   zero-fragment query as actionable failing evidence, and never advertises
   pixel precision. Blended transparency and unsampled motion remain outside it.
...
presented stabilized capture. Its one hashed cache
...
documents. Localization-aware game-font shaping remains.
...
Vehicle contexts additionally require the current one-session five-state motion
battery described in Offset Studio; one favourable held frame can no longer
approve car, hovercraft, or plane fit.
...
Richer reference animation, authored joint-limit inspection, and an explicit
pole witness only if the solver gains an authored pole remain.
...
Select fit approval retains the source-and-fit-bound v2 contract. Vehicle fit
approval uses v3 and additionally requires a current five-state representative
motion battery bound to source, fit, LOD policy, presentation, and the review
contract. Both require
a current warmed renderer result, valid fit/camera/contact/vehicle/visibility
...
Named-draft v14 and Fit-history v5 preserve the new meaning; v13/v4 preserve a
valid select approval but reopen every vehicle approval instead of silently
upgrading one parked frame into a five-state review.
