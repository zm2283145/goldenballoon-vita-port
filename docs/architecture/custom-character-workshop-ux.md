...
4. **A creator makes the character recognizable in every UI.** One identity
   ...
   same 40x40 runtime canvas. Eligible covered Unicode names show exact native
...
10. **A player leaves while work is running.** Changing launcher destinations
    never strands a completed result. Play and new imports remain unavailable
    while a character transaction owns the catalog. Quit or window-close keeps
    the launcher responsive, names the operation still settling, offers **Keep
    launcher open**, and publishes Quit only after the UI thread has received
    the result. The destructor join is a final process-safety net, never the
    ordinary invisible shutdown experience.
...
- Closing, changing pages, losing renderer support, or failing validation never
  ...
  revision.
- Background result publication is launcher-global rather than Workshop-page
  local. A pre-existing Play validation and a same-frame dropped source are
  arbitrated after the frame, so the engine cannot enter while a new character
  transaction owns the catalog.
...
Covered extended Latin, Greek, Cyrillic, Arabic, Hebrew, combining sequences,
and mixed-direction text use the same project-owned bounded rasterizer as the
live roster. HarfBuzz performs pinned OpenType shaping, SheenBidi resolves the
Unicode bidirectional runs, and subsetted OFL Roboto/Noto faces provide the
glyphs; no host font or ROM supplies them. The preview shows the actual
16-pixel RGBA output and cluster-safe compact-width ellipsis. Printable ASCII
intentionally keeps the retail font. Missing glyphs, unsafe invisible direction
...
one-level package finds retained raw drafts by exact package ID and routes the
author directly to the source choice. One same-ID candidate is safe to open
without mutation; multiple
matches require an explicit model-path-and-draft choice, and no match offers a
new original-GLB handoff. Installed-package and raw-model fingerprints are
different provenance domains, so the launcher never guesses by recency or
claims that package-ID routing proves source lineage. Model fingerprint review
remains mandatory, and the launcher never pretends a runtime slider can
...
- App-spoken focus guidance includes state and consequence: “Car fit, review
  required, opens preview.” It is not a native assistive-technology semantic
  tree and is never presented as screen-reader compatibility.
...
outside it. `not_available` is legal only for an unavailable input modality
with a reason and another platform must supply an observed pass; required
accessibility features, journeys, and views cannot be waived. The receipt names
the app's spoken guidance and never misrepresents it as a native screen-reader
tree. A performance exception remains visibly distinct from a pass and requires
a reason.
...
| Packaging/update/removal | Mutation-free review plus package/base-bound transactional install/update, responsive installed-versus-candidate diff, reversible runtime disable, authenticated revision restore/source export, current-compiler portable export, transactional current-source rebuild, and recoverability-aware deletion are executable. Recipient review leads with receiving-build compatibility, portable/source mode, new/update/same-source relationship, donor authority, WebGPU/OpenGL behavior, rig readiness, LOD count, and an explicitly unmeasured performance estimate before the exhaustive diff; keyboard/speech users receive the same evidence on the focusable rights control before consent. Candidate protocol v6 additionally carries bounded authored/generated tangent-basis counts for locally compiled source review and separates repaired/degenerate/fallback vertices from the fallback subset that affects normal-mapped material; the visual and spoken review gives a concrete UV/export/retest action. V3-v5, portable MDKC-v2, and installed-cache-only summaries disclose that this authoring provenance is unavailable rather than fabricating it; their repaired runtime tangent data remains intact. Every external compiler/manager operation and both portable/source-only reviewed installs acknowledge immediately and complete off the UI thread; the selected package/tab and playable last-known-good character remain stable until publication. One exception-safe worker lifecycle owns join/result/exception publication. It is serviced from every launcher destination; Play/import are gated while it owns the catalog, and normal Quit/window-close remains visibly cancellable until settlement. Portable publication refuses overwrite and does not mutate installed state; rebuild preserves enabled state and the last known-good cache on failure. Named full-editor snapshots are bounded, integrity-bound, atomically persisted, independently resumable/deletable, exact-base locked, and autosaved. Their separate bulk-transfer workflow exports all current-base snapshots into one exclusive-create `.mdkrdrafts` file after artwork/name sharing confirmation; source paths are removed and models, packages, licenses, ROM/save data, and performance evidence are omitted. Recipient review is mutation-free and discloses exact-source compatibility, additions, duplicates, and safe id renames through visual and speech paths. Confirmed import re-reads the reviewed digest and atomically adds without overwrite, rebase, resume, build, activation, or assignment. A combined optimistic build publishes portrait/profile/rig as one retained revision; launcher-owned fit is separately disclosed and confirmed. Permanent removal requires a confirmed-durable `armed` marker before native mutation, moves retained history before the cache commit witness, and retains a `retired` marker until private trash, preferences, named drafts, and exact-test evidence all reconcile. Restart recovery either restores a still-discoverable package without overwrite or completes retirement of the exact package-owned set, so a crash or cleanup failure retains an explicit retry path. A dedicated responsive launcher destination now owns a persistent library rail, eight editor tabs (including distinct Gameplay and Offset Studio jobs), named readiness rows, a deterministic best-next-action resolver, persisted package/tab selection, safe DAE/ZIP-to-GLB handoff, a multi-draft pre-package GLB authoring library, and independent bounded history for Identity, Profile, Rig, Fit/review, Performance, and Test setup; Settings retains its shortcut and assignment summary. | Observe cross-device draft transfer during the maintained human/device qualification matrix |
| Accessibility | Dedicated panel/tab/library/import/assignment controls carry spoken names, textual status, keyboard/controller navigation, and a narrow single-column fallback. Spatial placement/yaw/contact canvases expose spoken values plus adjacent navigable nudge/reset and exact numeric controls, and reflow instead of forcing a wide canvas. Because independently scrolling Library and Editor children cannot rely on framework sibling traversal, Ctrl/Cmd+I and controller Back/View provide a visible, globally routed, non-mutating jump to the source chooser; Esc and controller B return focus to the persistent launcher action row when no modal or text edit owns the input. Production SDL keyboard and virtual-controller tests prove both focus transitions before ordinary navigation resumes. The exact-test matrix, baseline/destructive controls, responsive player layout and held/transition mode, named A/B semantics and numeric phases, held presets, camera presets/numeric sliders, named character light, capture-disabled explanation, capture tray, report export/removal actions, and all four responsive inspection actions carry spoken labels and textual state. Rendered 200% keyboard/speech lifecycle gates cover held mode; a separate virtual-controller D-pad sweep covers transition mode through the production SDL/ImGui route and proves disabled still-capture controls leave the focus graph. Shared cards flatten keyboard navigation so nested actions remain reachable. | Run observed controller/spoken-guidance review and qualify remaining freeform editor gestures; the automated Animation Studio traversal is complete. Native screen-reader semantics remain outside the advertised product contract |
...
2. Constraint and secondary-motion authoring is implemented end to end,
   ...
   exact result-v23 feedback. Observed controller/spoken-guidance review of the
...
3. ROM-independent native glyph rendering is executable for covered extended
   Latin, Greek, Cyrillic, Arabic, Hebrew, combining sequences, and mixed bidi
   names. Generated Roboto/Noto subsets and the HarfBuzz/SheenBidi source
   archives have pinned source/tool hashes and offline-cache support; the same
   bounded rasterizer drives live roster tiles, selected-name display, and the
   Workshop's exact pixel preview. Plain ASCII deliberately retains the retail
   font. Unsafe direction controls, missing glyphs, and malformed UTF-8 fail
   closed to the existing deterministic retail projection with a named reason.
4. The collection path for a maintained device-profile corpus is executable.
   It exports only a complete, current, single-build/device matrix after an
   explicit identity disclosure, retains honest over-target rows, and omits
   character identity/content and raw workload digests. Representative high-,
   mid-, and low-tier physical observations still have to be collected and
   maintained; no generated fixture or one fast machine is relabelled as that
   corpus. Recorded optional offline simplification and projected-height
   thresholds with per-view hysteresis are complete; their explicit
   legacy-distance fallback remains part of the supported fail-visible
   contract.
...
| Create recognizable identity everywhere | Executable for the audited select/HUD/results/rankings/minimap/collection surfaces, exact 40x40 Portrait Studio, and ROM-independent native Latin/Greek/Cyrillic/Arabic/Hebrew shaping and bidi with exact Workshop preview | Reviewed embedded OFL faces for additional scripts and ownership declarations for future surfaces |
...
| Leave while work is running | Executable: launcher-global publication keeps destination changes safe; catalog ownership gates Play and new imports; Quit/window-close stays responsive, names the settling operation, defaults to keeping the launcher open, and publishes Quit only after the UI thread receives the result | Observed window-manager and controller/spoken-guidance qualification on every release platform |

Every story remains subject to observed mouse, keyboard, controller, touch,
app-spoken-guidance, 200% scale, narrow-layout, reduced-motion, and colour-vision
qualification. Automated keyboard/speech/touch and responsive gates are strong
regression evidence, not a substitute for that M8 human release gate. Native
screen-reader semantics are not claimed by this ImGui shell.
...
and its compact label has a rendered containment gate. When a dense editor's
independent scroller owns focus, Ctrl/Cmd+I or controller Back/View moves focus
to the safe source chooser without opening a picker or mutating state; rendered
SDL keyboard and virtual-controller gates require its spoken focus result. Esc
or controller B likewise returns focus to the persistent launcher action row
without overriding a modal or active text edit. An explicit no-overwrite
DAE/ZIP-to-GLB handoff covers ordinary
...
remain user-owned documents. Additional script coverage is data-driven through
reviewed embedded OFL faces; the shaping and bidi runtime is already shared by
Workshop and game.
...
Further bespoke scene-owned conditions and representative physical-device
profile collection remain. Recorded offline simplification, projected-height transition
...
available. Additional script coverage is data-driven through reviewed embedded
OFL faces; it does not require another runtime architecture or host fonts.
