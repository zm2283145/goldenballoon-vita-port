...
confirm in its authored room. Each vehicle automatically runs three exact
one-player sessions: an open baseline, dense scenery, and an alternate
environment, mapped to qualified courses for that vehicle. Every session covers
...
1. Donor-reference occlusion, named attachment attribution, weather/vehicle
   conditions beyond the qualified open/dense/alternate course batteries, and
...
model-only stabilized PNG products, and self-contained schema-v4 qualification
...
Further weather/vehicle-condition variants, transition visualization, a
maintained device profile corpus, projected-size LOD hysteresis and optional
recorded offline simplification remain.

Select and vehicle fit approval use the source-and-fit-bound v5 scene-review
contract. Select requires its current three-state room battery. Each vehicle
requires all eleven race samples on open, dense, and alternate-environment
qualified courses, bound to source, fit, LOD policy, presentation, and review
contract.
Both require
...
Named-draft v16 and Fit-history v7 preserve the new meaning. Every older
approval reopens once rather than silently upgrading fixed-scene evidence into
the multi-scene contract.
