...
| Preview | One-click typed requests launch select or all three vehicles through real game initialization in any 1-4P layout after ROM revalidation; requests are assignment-neutral and support all 13 select/race semantics held at an exact normalized phase. Pose inspection is typed through the launcher/game/runtime contract, counted after warm-up, usable as exact session fit proof, saved in Test history and named drafts, and explicitly excluded from durable performance evidence. Live tests return a warmed structured result with exact device and physical render identity, persist source/tuning-bound per-context author review plus the bounded 4x4 latest/baseline evidence matrix, and a generated non-Diddy package passes the complete WebGPU route/pixel/stress/result gate. | Embed the renderer and add camera/lighting controls plus screenshots/contact sheets |
...
| Accessibility | Dedicated panel/tab/library/import/assignment controls carry spoken names, textual status, keyboard/controller navigation, and a narrow single-column fallback. The exact-test matrix, baseline/destructive controls, semantic pose selector, phase scrubber, and all four responsive inspection actions have a rendered 200% keyboard/speech lifecycle gate. | Qualify every remaining editor tool with controller/screen reader and close spatial-tool gaps |
...
and lighting controls, and contact overlays remain. The launcher-owned Test
