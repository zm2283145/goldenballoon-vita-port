...
version-nine named drafts. Static-looking active mappings remain an explicit
...
midpoint phase in Test; wide screens use a comparison table and narrow screens
preserve the same controls as sequential cards. The editor uses recognizable
...
instruction to re-enable every clip. The next autosave upgrades that decision
explicitly to version nine.
