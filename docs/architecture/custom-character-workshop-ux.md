...
| Height/front/ground/seat calibration | Executable v2 calibration plus direct per-context translation/yaw controls and exact game-renderer target-frame floor/seat/bounds/facing diagnostics retained across restart. The current editor plane and the full Test result render the authoritative calibrated volume, fitted ground/seat anchor after authored corrections, zero datum, and measured forward direction as focusable front/side/top overlays with an equivalent spoken/numeric description. | Add an exact embedded model preview so the renderer-produced character image can sit behind the already-authoritative overlays |
| Per-context corrections | One package-keyed editor shared by every local assignment, with independent select/car/hovercraft/plane transforms; synchronized front/side/top ground-or-seat translation pads; a target-space yaw dial; numeric fields; per-plane reset/nudge; explicit undoable vehicle-fit copying; source-bound gesture undo/redo; source/fit-bound durable exact renderer measurements; and a dynamic selected-plane overlay of the last matching exact measurement. Stale fit evidence is never drawn against changed controls. | Add exact model and donor ghosts behind the authoritative bounds/anchor layer |
