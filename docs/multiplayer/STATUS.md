...
| `MDKR_ONLINE_LOBBY_DROP_GRACE` | 2 | Authored ticks of peer silence a room departure waits out before it is acted on. An authenticated packet from the departed endpoint inside the grace drops the verdict for that race and hands the loss back to the transport ladders, so a room-service wobble cannot produce a false "opponent left" while the peer link is fine. A held verdict is terminal for that race: the same member cannot be dropped by the room again, so a peer that wobbles and then genuinely quits is resolved by the transport ladders (20-30 s) rather than by the room. Set `0` to act on the verdict the moment it arrives (the pre-grace behaviour, and the lane's positive control). A value above 30, or one that is not a plain number, is ignored and the default of 2 stands -- lengthening the grace is not how the drop is switched off. |
