...
| N5-D | **A race can start before the route has been measured, and it is never held for one.** Route quality measures for 6 s and drains for 1 s (`MDKR_MATCH_ROUTE_MEASURE_MS` / `_DRAIN_MS`). Start before it settles and the race runs on the manifest floor delay, with the room chip saying the connection is being checked. | **DECIDED (measure from first open, never block Start)** | The window now opens at the FIRST peer channel rather than the last, so it normally closes long before a human reaches Start; probes only ever flow over channels that are open (the transport's unit of readiness is the peer, so the addressed control lane is filtered to ready peers and the broadcast bundle lane is the mesh's own send). Start is never held: the chip reads “Checking connection…” and entry timing falls back to the agreed floor. A window Start cuts short is scored, not discarded — the probes still in flight at the cut are dropped rather than counted as loss — and its record is still exchanged as the round's second attestation, so the NEXT race of a tournament gets the widen and the chip. The echo budget is 16 per pump, so a launcher hitch inside the now busier window cannot move anyone's chip. Gates: `online_live_route`'s start-before-settle arm, `match_route_quality`'s cut arm, the view model's checking chip. |
