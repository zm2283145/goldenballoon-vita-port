...
Some of its consequences are owner decisions or release copy rather than code,
...
|---|---|---|---|
| N8-D | **Membership decides a mid-race drop, and it decides it fast — but only when the peer link agrees.** A player whose signaling socket drops mid-race is finalised and dropped by the survivor within about four authored ticks, where the previous regime waited for the transport's own ladders (the 20 s control-ping bound, or ICE teardown plus the 10 s vanish dwell). `MDKR_ONLINE_LOBBY_DROP=0` restores that regime. | **DECIDED (grace rule)** | The wobble half of the trade is closed. The room's verdict is still the trigger, but it now waits out a two-authored-tick peer-silence grace: any authenticated packet from the departed endpoint inside the grace holds the verdict for that race and hands the loss back to the transport ladders, so a room-service wobble on a healthy peer link can no longer produce a false "opponent left". A real quit closes the peer's channels too, so it stays silent through the grace and the fast card is preserved. `check_online_lobby_drop.py` measures all four regimes against each other: the drop, the plumbing off, the wobble held, and the wobble falsely dropped with `MDKR_ONLINE_LOBBY_DROP_GRACE=0`. Two residuals remain for the owner, below. |
| N8-D-r1 | **A held verdict is terminal for that race.** Once the grace drops a verdict, the same member cannot be dropped by the room again in that race: a peer that wobbles, is held, and then genuinely quits is resolved by the transport ladders (the 20 s control-ping bound, or ICE teardown plus the 10 s vanish dwell) rather than by the room. Also, a peer whose input happens not to land in the two graced ticks — a send hiccup arriving beside the room's verdict — is still dropped on the room's word. | **RESIDUAL (accept or retune)** | Deliberate: re-arming on every fresh verdict would let a flapping room retry the drop until one grace fell in a gap between the peer's bundles, which is the false drop wearing a different hat. The cost if the answer is wrong is a slow card for the wobble-then-quit sequence, not an incorrect one. `MDKR_ONLINE_LOBBY_DROP_GRACE` is the field retune; the shape of a real fix is to re-arm only on a verdict that arrives after a further interval of peer silence, which needs a second silence measurement this layer does not take today. |
| N8-D-r2 | **With three or more endpoints, a hold is a local decision and two survivors can disagree.** If the proposer heard silence while another survivor heard the peer, the proposer proposes a tick, finalises the seat and ends its session; the survivor that heard the peer has no room verdict left to intersect (the hold erased it), refuses the proposal, races on, and then loses the proposer itself to the 20-30 s ladders. The two commit different canonical input for that seat in between. | **RESIDUAL (must be answered before `GO`)** | A hold by the *proposer* is unanimous by construction (it proposes nothing, so no survivor has a tick to finalise at), and 2P — the only shape any end-to-end lane in this tree runs — cannot reach the asymmetry at all, because the sole survivor is always the proposer. So this is unreachable until 3-4P racing is exercised for real, which is also the gap N8 left open (its ≥3P proposal path is unit-tested and correct by construction, with no engine-level lane). Fix shape: carry a silence witness in the `race_drop` proposal — the proposer's own graced tick span and packet count — so a recipient can tell a proposal made in silence from one made blind; a true hold *agreement*, where no survivor acts unless all of them heard silence, needs a round trip this layer does not have and should not be bolted on without a ≥3P lane to measure it. |
| N5-D | **A race can start before the route has been measured, and it is never held for one.** Route quality measures for 6 s and drains for 1 s (`MDKR_MATCH_ROUTE_MEASURE_MS` / `_DRAIN_MS`). Start before it settles and the race runs on the manifest floor delay, with the room chip saying the connection is being checked. | **DECIDED (never block Start; score what the start cut short)** | The window opens at the FIRST peer channel rather than the last. Be precise about who that helps: at 3-4P it removes the wait for the slowest peer; **at 2P — the only shape any lane here runs — it changes nothing**, because the single peer's `PeerChannelsReady` is both first and last open and the 7 s wall stands where it did. Whether 2P is usually settled before Start rests on the human time at the phrase comparison, not on when the window opens. What D2 delivers for 2P is the cut path: Start is never held, the chip reads “Checking connection…”, entry timing falls back to the agreed floor, and the window is scored rather than abandoned — the probes younger than the cut margin (500 ms, or twice the measured p95 on a slower route) are dropped rather than counted as loss. A cut record is adopted only with 30 answered samples spanning 2 s of window, else it is discarded and the next round measures fresh; an adopted cut record is exchanged as the round's second attestation and then retired at the next race-latch reset (a FULL window's record is held for the tournament — the lanes are idle in the lobby, so re-measuring after a cut costs nobody a wait). Probes only ever flow over channels that are open. The echo budget is 16 per pump. Gates: `online_live_route`'s start-before-settle and next-round arms, `match_route_quality`'s cut/margin/floor arms, the view model's checking chip, `online_live_adapter_beta`'s boundary seam. |
