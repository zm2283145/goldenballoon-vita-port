...
#include "gbi.h"
#include "audiosfx.h"
#include "dkr_native_ptr.h" /* NATIVE_PORT: dkrptr32 slots for ROM-overlaid structs */
...
#ifdef NATIVE_PORT
    /* 0x0C */ dkrptr32 cmd; // runtime Gfx* token (arena DL heap); keeps 0x20 layout
#else
...
#ifdef NATIVE_PORT
DKR_ASSERT_OFFSET(TextureHeader, cmd, 0x0C);
DKR_ASSERT_OFFSET(TextureHeader, numOfTextures, 0x12);
DKR_ASSERT_OFFSET(TextureHeader, textureSize, 0x16);
DKR_ASSERT_OFFSET(TextureHeader, isCompressed, 0x1D);
DKR_ASSERT_SIZE(TextureHeader, 0x20);
#endif
...
typedef struct LevelHeader {
    ...
    /* 0x16 */ u8 unk16[10];
#ifdef NATIVE_PORT
    /* 0x20 */ dkrptr32 AILevelTable; /* inline AI-level bytes; only &-addressed */
#else
    ...
    union {
#ifdef NATIVE_PORT
        /* 0x70 */ dkrptr32 unk70[1]; // LevelHeader_70* token; only size of 1 matches
#else
...
#ifdef NATIVE_PORT
    /* 0x74 */ dkrptr32 unk74[7]; /* LevelHeader_70* tokens (misc-asset idx→ptr) */
#else
    ...
    /* 0xA3 */ s8 unkA3;
#ifdef NATIVE_PORT
    /* 0xA4 */ dkrptr32 unkA4; /* TextureHeader* token (specialSkyTexture) */
#else
    ...
    /* 0xAA */ s16 unkAA;
#ifdef NATIVE_PORT
    /* 0xAC */ dkrptr32 pulseLightData; /* PulsatingLightData* token */
#else
...
#ifdef NATIVE_PORT
/* On-disk (N64) layout locks — offsets from levelHeader.hpp / structs.h. */
DKR_ASSERT_OFFSET(LevelHeader, course_height, 0x08);
DKR_ASSERT_OFFSET(LevelHeader, AILevelTable, 0x20);
DKR_ASSERT_OFFSET(LevelHeader, geometry, 0x34);
DKR_ASSERT_OFFSET(LevelHeader, instruments, 0x54);
DKR_ASSERT_OFFSET(LevelHeader, waveViewDist, 0x6E);
DKR_ASSERT_OFFSET(LevelHeader, unk74, 0x74);
DKR_ASSERT_OFFSET(LevelHeader, weatherEnable, 0x90);
DKR_ASSERT_OFFSET(LevelHeader, unkA4, 0xA4);
DKR_ASSERT_OFFSET(LevelHeader, pulseLightData, 0xAC);
DKR_ASSERT_OFFSET(LevelHeader, BGColourBottomR, 0xBE);
DKR_ASSERT_SIZE(LevelHeader, 0xC4);
#endif
...
#ifdef NATIVE_PORT
    /* 0x00 */ dkrptr32 texture; /* TextureHeader* token (offset→ptr at load) */
#else
...
#ifdef NATIVE_PORT
DKR_ASSERT_OFFSET(TextureInfo, width, 0x04);
DKR_ASSERT_SIZE(TextureInfo, 0x08);
#endif
...
typedef struct ObjectModel {
#ifdef NATIVE_PORT
    /* 0x00 */ dkrptr32 textures;
    /* 0x04 */ dkrptr32 vertices;
    /* 0x08 */ dkrptr32 triangles;
    /* 0x0C */ dkrptr32 collisionFacets;
    /* 0x10 */ dkrptr32 collisionPlanes;
    /* 0x14 */ dkrptr32 attachPoints;
#else
    ...
    /* 0x1A */ s16 unk1A;
#ifdef NATIVE_PORT
    /* 0x1C */ dkrptr32 collisionSpheres;
#else
    ...
    /* 0x34 */ u8 pad34[4];
#ifdef NATIVE_PORT
    /* 0x38 */ dkrptr32 batches;
    /* 0x3C */ f32 unk3C;
    /* 0x40 */ dkrptr32 normals;
    /* 0x44 */ dkrptr32 animations;
#else
...
#ifdef NATIVE_PORT
    /* 0x4C */ dkrptr32 animatedVertexIndices;
#else
...
#ifdef NATIVE_PORT
DKR_ASSERT_OFFSET(ObjectModel, numberOfAttachPoints, 0x18);
DKR_ASSERT_OFFSET(ObjectModel, collisionSpheresSize, 0x20);
DKR_ASSERT_OFFSET(ObjectModel, numberOfBatches, 0x28);
DKR_ASSERT_OFFSET(ObjectModel, fileSize, 0x2C);
DKR_ASSERT_OFFSET(ObjectModel, batches, 0x38);
DKR_ASSERT_OFFSET(ObjectModel, numberOfAnimatedVertices, 0x4A);
DKR_ASSERT_OFFSET(ObjectModel, animatedVertexIndices, 0x4C);
DKR_ASSERT_OFFSET(ObjectModel, hasAnimatedTexture, 0x50);
#endif
...
#ifdef NATIVE_PORT
    /* 0x00 */ dkrptr32 vertices;
    /* 0x04 */ dkrptr32 triangles;
    /* 0x08 */ s32 unk8;
    /* 0x0C */ dkrptr32 batches;
    /* 0x10 */ dkrptr32 unk10;
    /* 0x14 */ dkrptr32 collisionFacets;
    /* 0x18 */ dkrptr32 collisionPlanes;
#else
...
#ifdef NATIVE_PORT
DKR_ASSERT_OFFSET(LevelModelSegment, unk8, 0x08);
DKR_ASSERT_OFFSET(LevelModelSegment, numberOfVertices, 0x1C);
DKR_ASSERT_OFFSET(LevelModelSegment, numberOfBatches, 0x20);
DKR_ASSERT_OFFSET(LevelModelSegment, unk2C, 0x2C);
DKR_ASSERT_OFFSET(LevelModelSegment, unk3C, 0x3C);
DKR_ASSERT_SIZE(LevelModelSegment, 0x44);
#endif
...
#ifdef NATIVE_PORT
    /* 0x00 */ dkrptr32 textures;
    /* 0x04 */ dkrptr32 segments;
    /* 0x08 */ dkrptr32 segmentsBoundingBoxes;
    /* 0x0C */ dkrptr32 unkC; // Unused?
    /* 0x10 */ dkrptr32 segmentsBitfields;
    /* 0x14 */ dkrptr32 segmentsBspTree;
#else
    /* 0x00 */ TextureInfo *textures;
...
#ifdef NATIVE_PORT
DKR_ASSERT_OFFSET(LevelModel, numberOfTextures, 0x18);
DKR_ASSERT_OFFSET(LevelModel, numberOfSegments, 0x1A);
DKR_ASSERT_OFFSET(LevelModel, minimapSpriteIndex, 0x20);
DKR_ASSERT_OFFSET(LevelModel, modelSize, 0x48);
DKR_ASSERT_SIZE(LevelModel, 0x4C);
#endif
...
typedef struct ObjectHeader {
    ...
    /* 0x0C */ f32 scale;
#ifdef NATIVE_PORT
    /* 0x10 */ dkrptr32 modelIds;
    /* 0x14 */ dkrptr32 vehiclePartIds;
    /* 0x18 */ dkrptr32 vehiclePartIndices;
    /* 0x1C */ dkrptr32 objectParticles;
    s32 pad20;
    /* 0x24 */ dkrptr32 unk24;
#else
    /* 0x10 */ s32 *modelIds;
...
#ifdef NATIVE_PORT
DKR_ASSERT_OFFSET(ObjectHeader, modelIds, 0x10);
DKR_ASSERT_OFFSET(ObjectHeader, unk24, 0x24);
DKR_ASSERT_OFFSET(ObjectHeader, flags, 0x30);
DKR_ASSERT_OFFSET(ObjectHeader, numberOfModelIds, 0x55);
DKR_ASSERT_OFFSET(ObjectHeader, internalName, 0x60);
DKR_ASSERT_SIZE(ObjectHeader, 0x78);
#endif
...
#ifdef NATIVE_PORT
    /*
     * Runtime-sized wave samples follow the fixed header. The matching layout
     * spells the first two bytes as an array; native code must use a flexible
     * member so every allocated element is inside the C object.
     */
    /* 0x0E */ s8 unkE[];
#else
    /* 0x0E */ s8 unkE[2];
...
#ifdef NATIVE_PORT
    /* LP64: this slot stores a real host `AudioPoint *`, not a 32-bit N64 address.
     * obj_loop_snowball() hands `&soundMask` to audspat_play_sound_at_position() as
     * an `AudioPoint **`, and audspat_point_create() stores the pointer through it
     * (`*handlePtr = audioPoint`, audio_spatial.c) *and* keeps that same address in
     * `AudioPoint::userHandlePtr` to write back through when the point is released.
     * With the decomp's `u32` an 8-byte store lands on a 4-byte field, which on this
     * host does two things at once: the value read back is truncated to its low 32
     * bits (a wild pointer — the level-9 SIGSEGV), and the high 32 bits spill over
     * `currentSound`/`unk26`, silently rewriting the snowball's sound id. Every other
     * `soundMask` in the codebase is already a real `struct AudioPoint *`; this was
     * the only one left as an integer. This struct is runtime-only — it is carved out
     * of the object blueprint by get_object_property_size(), never overlaid on ROM
     * bytes — and it already differs from the N64 layout because unk18/unk1C are
     * host pointers, so widening this field costs nothing but 8 bytes of pool. */
    /* 0x20 */ struct AudioPoint *soundMask;
#else
    /* 0x20 */ u32 soundMask;
#endif
