...
typedef struct SaveConfig {
    ...
    unsigned language : 2;
    TT_COURSES
    /* Bits 24 and 25 are distinct. The original declaration omitted bit 24,
     * so its `subtitles` name described the default-high flag while every
     * runtime user correctly operated on bit 25. */
    unsigned defaultHigh : 1;
...
_Static_assert(sizeof(SaveFile) == 40, "SaveFile must match one EEPROM slot");
_Static_assert(sizeof(SaveConfig) == 8, "SaveConfig must match one EEPROM block");
