...
/*
 * Native-only display-space tags carried in the two low, otherwise-unused bits
 * of F3DDKR's matrix parameter byte.  The high two bits remain the original
 * matrix slot, so this costs no extra display-list command and no arena space.
 *
 * INHERIT is zero deliberately: every original/model matrix command keeps the
 * current draw policy.  Only the camera's base world/ortho matrices opt into a
 * new policy.  Non-native builds strip the tag completely.
 */
#define G_MTX_DKR_SPACE_INHERIT   0
...
/*
 * Native renderer metadata. F3DDKR does not assign MoveWord indices 0x0C/0x0E;
 * the original microcode ignores them. The native game uses them to retain
 * per-object remaster classification and normal-stream identity in the queued
 * display list. They never enter ROM assets and non-native builds never emit
 * them.
 */
