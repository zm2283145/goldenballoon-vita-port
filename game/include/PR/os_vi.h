...
/*
 * os_vi.h — clean-room video-interface declarations.
 *
 * Declares the per-mode VI register tables (NTSC/PAL/MPAL/FPAL, each
 * in progressive and interlaced, standard- and high-res variants) and
 * the calls this port uses to select a mode, swap and fade
 * framebuffers, and toggle special display features like gamma
 * correction and the dither filter. This is a declaration-only
