...
/*
 * os_message.h — clean-room message queues, events and thread signalling
 * declarations.
 *
 * Declares the fixed-capacity message queue type and the operations to
 * send, jam (priority-send), and receive messages on it, plus the hardware
 * event codes and the call that wires an event to a queue. Declaration-only
...
typedef struct OSMesgQueue_s {
    OSThread    *mtqueue;       /* threads waiting to receive from an empty queue */
    OSThread    *fullqueue;     /* threads waiting to send to a full queue */
