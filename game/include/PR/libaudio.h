/*
 * libaudio.h — clean-room audio library declarations.
 *
 * Declares the audio library's core types and entry points: sound bank and
 * wavetable data layouts, the synthesizer driver (ALSynth) and its voice
 * allocation calls, the event queue, and the sequence/compressed-sequence
 * and sound players built on top of it. This is a declaration-only
...
#ifdef NATIVE_PORT
/* M5 audio — big-endian, LP64-safe parsers (bnkf.c). See that file. */
extern ALHeap *gAudioParseHeap;  /* arena heap the parsed banks use */
extern u32 gAudioBankCtlSize;    /* raw ctl image size for bounds    */
...
#ifdef NATIVE_PORT
/*
 * NOT a real event type. alEvtqNextEvent() reports "the queue is empty" by
 * writing this sentinel into the caller's ALEvent and returning a delta of 0
 * (see game/libultra/src/audio/event.c and SGI's 11/28/95 comment there).
 *
 * Every player's voice handler is a `do { switch (nextEvent.type) ... } while
 * (nextDelta == 0);` loop, and none of the switches has a case for the
 * sentinel, so an empty queue makes the handler spin forever -- and because
 * __allocVoiceHandler() advances client->samplesLeft by the handler's return
 * value, returning 0 would wedge the caller's for-loop too. Handlers must
 * therefore detect this value explicitly and return a nonzero delta.
 * See docs/OPEN_ITEMS.md ("attract demo hangs in the audio sequence player").
 */
#define AL_EVTQ_EMPTY_EVT (-1)
#endif
