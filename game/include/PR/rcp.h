...
/*
 * rcp.h — clean-room Reality Coprocessor register addresses, bit fields
 * and DMA constants.
 *
 * Declares the memory-mapped register offsets and bit patterns for the
 * RCP's RDRAM, SP, DP, MI, VI, AI, PI, RI and SI blocks, along with the
 * DMA and controller-channel constants this port needs to talk to that
 * hardware. It is a declaration-only compatibility surface: no vendor
 * implementation is present, only the layout the rest of the port relies on.
 */
...
/**
 * Controller channel
 * Bits 6-7 of the Rx and Tx data-size bytes hold 4 error flags per
 * channel. When writing a new Tx/Rx size value for a channel, be sure
 * to clear these bits first.
 */
...
/**
 * The address and length registers each have two buffer slots, so a
 * second write can be queued before the first is consumed. For a given
 * transfer, write the address register first and the length register
 * second.
 */
...
/* Writing to the PI status register only looks at 2 bits:
 *  Bit 0: reset the PI controller
 *  Bit 1: clear the pending interrupt
 * Both bits may be set at once to reset and clear together.
 *
 * Behavior to keep in mind:
 *  - Every completed DMA raises an interrupt; an interrupt handler must
 *    clear it explicitly by writing bit 1 to this register.
 *
 *  - A new request can be issued as soon as the controller is idle,
 *    even if the previous interrupt hasn't been cleared yet — it's up
 *    to the caller to keep DMA completions and interrupt clears in sync.
 *
 *  - Resetting a busy controller still produces the interrupt once it
 *    goes idle; resetting an already-idle controller does not.
 */
