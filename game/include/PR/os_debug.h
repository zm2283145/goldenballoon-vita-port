...
/*
 * os_debug.h — clean-room assertion and debug-print entry points.
 *
 * Declares the sampling-profiler controls (init/start/stop/flush) and the
 * per-thread profiling counters used to measure CPU time spent in each
 * thread, for this port's debug tooling. Declaration-only compatibility
 * surface; no vendor implementation is present.
 */
