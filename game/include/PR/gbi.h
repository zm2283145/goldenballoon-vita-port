/*
 * gbi.h — clean-room graphics binary interface declarations.
 *
 * This header defines the display-list command macros, RDP/RSP state
 * constants, and packed-struct types that the decompiled render code in
 * this port compiles against. It covers matrix, vertex, and display-list
 * commands as well as the tile, combiner, and other-mode state used to
 * program the rasterizer. It is a declaration-only compatibility surface
 * reconstructed for this project — no vendor microcode or library
...
/*
 * Command byte layout, encoded in the top two bits of the opcode:
 *
 *	|00xxxxxx| = DMA		0,..,127
 *	|10xxxxxx| = Immediate Mode	-65,..,-128
 *	|11xxxxxx| = RDP cmds		-1,..,-64
 *
 * G_RDP_ADDR_FIXUP marks the boundary between RDP commands that carry an
 * embedded DRAM address needing relocation and those that don't, so a
 * single fixup path can handle every command below it.
 *
 * The category names don't map cleanly onto behavior:
 *
 *	- 'DMA' commands reference additional data elsewhere in memory and
 *	  trigger a transfer of that data into DMEM.
 *
 *	- 'Immediate' commands are self-contained: the whole command fits in
 *	  one 64-bit word, so it can be consumed without any extra memory
 *	  transfer, even though nothing about it is otherwise special.
 *
 *	- 'RDP' commands are commands that can be forwarded through the RSP
 *	  straight to the RDP. Confusingly, a few of the macros below that
 *	  are conceptually "DP" commands actually compile down to Immediate
 *	  commands rather than true RDP commands.
 *
 * A related family of RDP commands covers the triangle rasterization
 * primitives (with edge slopes, etc.) that follow the same command
 * ordering as the rest of this table.
 *
 * The low 6 bits of each RDP command byte are significant to the
 * rasterizer hardware, so this encoding is effectively fixed and any
 * change here has to track the hardware's expectations.
 */
...
/*
 * These triangle opcodes are emitted internally by the RSP microcode
 * rather than issued directly by calling code.
 *
 * The bit layout is chosen to minimize ucode work rather than for
 * readability: the id's run -56, -52, -54, -50, -55, -51, -53, -49, ...
 * with each bit flagging one of edge / shade / texture / zbuffer (estz).
 */
...
/*
 * TRI_FILL only rasterizes edges; the primitive color has to be selected
 * in the DP mode for anything to be visible. Despite the name, it is not
 * the RDP's 'fill mode' — triangles can't use that mode at all.
 *
 * TRI_SHADE is always Gouraud-interpolated. A "flat-shaded" triangle is
 * still processed as Gouraud, just with all three vertex colors equal and
 * zero color deltas.
 *
 * The remaining triangle variants combine these behaviors and are
...
/*
 * HACK:
 * Early RDP revisions could hang on certain 'bowtie' triangle shapes.
 * Raising this value works around it, at the cost of trimming one
 * scanline off every triangle for each increment of 4; 4, 8, or 12 is
 * typically enough headroom to avoid the hang. Any change here requires
 * recompiling every display list, including static ones, since it's
 * baked into the generated command data.
 */
#define BOWTIE_VAL	0
...
#define	GPACK_RGBA5551(r, g, b, a)	(((((u32)(r))<<8) & 0xf800u) | \
					 ((((u32)(g))<<3) & 0x7c0u) | \
					 ((((u32)(b))>>2) & 0x3eu) | (((u32)(a)) & 0x1u))
...
/*
 * Flags for G_SETGEOMETRYMODE (rendering state tracked by the RSP).
 *
 * The low 8 bits are reserved bit-for-bit: they get OR'd directly with
 * G_TRI_SHADE (11001100) to build the triangle opcode, so their positions
 * can't be changed. The high 8 bits are reserved too — they're OR'd with
 * 0x703 to form the clip code mask, where a value of 0x04 turns on near
 * clipping and zero turns it off.
 *
 * G_SHADE has to be set for per-vertex colors to show up at all;
 * otherwise the DP mode must be configured to fall back to primcolor.
 *
 * G_SHADING_SMOOTH selects Gouraud interpolation across all three vertex
 * colors. Without it, the triangle is flat-shaded: only one vertex color
 * is used, with the ucode forcing all three vertices to match it.
 */
 ...
 *
 * NOTE:
 * DMA transfers on this hardware move data in 64-bit-aligned, 64-bit-
 * sized chunks, which leaks into the API: several structures below carry
 * explicit padding and forced alignment to match.
 *
 * Because the compiler otherwise aligns a struct to its widest member,
 * 64-bit alignment can't be assumed automatically. It's forced here by
 * wrapping each struct in a union alongside a dummy `long long int`
 * member, a standard trick for getting predictable alignment out of a
 * plain-C compiler.
 *
 * The tradeoff is that code has to go through the union to reach the
 * real struct, but there's no space cost — every wrapped struct is
 * already at least 64 bits, so the alignment member doesn't grow the
 * union. Static initializers still work because a union initializes
 * from its first member, which is why the alignment field is always
 * declared last.
 */
...
/* LP64/ILP32 fix: N64 `long` is 32-bit, so the RSP matrix is 16x4 bytes = 64 B
 * and the RDP/RSP consumes those exact bytes (s15.16 split format). On LP64
 * `long` is 8 bytes, which would inflate Mtx to 128 B and desync BOTH the writer
 * (mtxf_to_mtx addresses the fraction half via &m->m[2][0]) and every consumer
 * of sizeof(Mtx) (gMatrixHeap/gTriangleHeap pool layout, the (*mtx)++ advance,
 * and the F3DDKR interpreter's 64-byte matrix read). Pin the element to a 32-bit
 * type so the on-wire layout matches N64 on every host width. */
...
/*
 * This is the largest integer screen-Z value the rasterizer can accept
 * (an additional 16 bits of fraction get added during transforms, but
 * the integer part is hard-capped at this). Any final z-coordinate that
 * exceeds it after transform and scaling is out of range and won't
 * rasterize correctly.
 *
 * It is not the same value as G_MAXFBZ, which is what you initialize the
 * z-buffer contents to. The two differ because of how the rasterizer's
 * internal z path is optimized; a more abstracted API could hide that
 * distinction, but this one exposes it directly.
 */
#define G_MAXZ		0x03ff	/* 10 bits of integer screen-Z precision */

/*
 * Viewport scale and translate values carry 2 bits of fraction, which
 * gives the hardware sub-pixel positioning and also lets it handle
 * viewports whose dimensions aren't evenly divisible.
 *
 * With the default projection and view matrices, and accounting for
 * those fraction bits, a viewport is typically initialized as:
 *
 ...
 * MOVEMEM indices
 *
 * Each value here selects an entry in a dmem table; that entry in turn
 * points at a 1-4 word region of dmem used as the destination for a
 * 1-4 word DMA transfer.
 */
 ...
 * MOVEWORD indices
 *
 * Each value here selects an entry in a dmem table; that entry points at
 * the single dmem word where an immediate value gets written.
 */
 ...
 * Light structure.
 *
 * Only directional (infinite) lights are supported. The field ordering
 * looks unusual because it's arranged to keep the microcode's dmem
 * layout aligned.
 */
...
#if defined(NATIVE_PORT)
	/* LP64 fix (same class as Mtx_t above): N64 `long` is 32-bit, so this RDP
	 * setcolor packet is 8 bytes and every command in the Gfx union is 8 bytes
	 * "by law" (see Gfx typedef). On LP64 `unsigned long` is 8 bytes with 8-byte
	 * alignment, which alone inflates sizeof(Gfx) from 8 to 16 — desyncing every
	 * DL layout that mixes a literal 8-byte Gfx stride (e.g. tex_load_sprite's
	 * `numTextures * 0x20` DL reservation = 4*sizeof(Gfx)) with the writer's
	 * sizeof(Gfx) advance: the DL region then over-runs into the following
	 * vertex region, so sprite quads read back Gfx-command bytes (billboard
	 * "red spikes"). `color` is never consumed as a struct member (all builders
	 * write words.w1), so pin it to 32 bits to keep the on-wire 8-byte packet. */
		unsigned int	color;
#else
		unsigned long	color;
#endif
...
/*
 * The core display-list element type: every command occupies exactly one
 * 64-bit slot, regardless of which variant is active.
 */
typedef union {
	Gwords		words;
	Gdma		dma;
...
#if defined(NATIVE_PORT)
/* LP64: the DL word is 32-bit, so `s` is truncated. When `s` is a raw 64-bit
 * host pointer (e.g. gSPDisplayList/gSPBranchList to a static rodata DL) the
 * high bits are unrecoverable, so register the full pointer for the renderer's
 * resolver. dkr_dl_register_host_ptr ignores values that already fit in 32 bits
 * (pre-computed OS_K0_TO_PHYSICAL / segment-base tokens). See stubs_dkr.c. */
extern void dkr_dl_register_host_ptr(const void *x);
#define	gDma1p(pkt, c, s, l, p)						\
{									\
	Gfx *_g = (Gfx *)(pkt);						\
	/* Evaluate `s` EXACTLY ONCE: it is frequently `OS_K0_TO_PHYSICAL	\
	 * ((*mtx)++)` / `(*vtx)++`, whose ++ side effect must not fire	\
	 * twice (that desynced the matrix pool by one Mtx and collapsed	\
	 * all geometry). */						\
	uintptr_t _s = (uintptr_t)(s);					\
	dkr_dl_register_host_ptr((const void *)_s);			\
	_g->words.w0 = (_SHIFTL((c), 24, 8) | _SHIFTL((p), 16, 8) |	\
			_SHIFTL((l), 0, 16));				\
	_g->words.w1 = (unsigned int)_s;				\
}
#else
#define	gDma1p(pkt, c, s, l, p)						\
{									\
...
/*
 * Note: gSP1Triangle() and the line macro scale each vertex index by 10
 * to match a microcode-side optimization.
 */
...
/*
 * Like gSPLine3D, but with an explicit width parameter. The line is
 * always at least 1.5 pixels thick, and this width is added on top of
 * that floor in half-pixel units — so a width of 1 gives a 2.0-pixel
 * line (1.5 + 0.5).
 */
...
/*
 * r must be one of FRUSTRATIO_1 through FRUSTRATIO_6.
 */
...
/*
 * Overwrite a single element of the currently loaded matrix.
 *
 * where = byte offset of the element within the matrix
 * num   = replacement 32-bit value (covers 2 integer or 2 fractional
 *         matrix components)
 */
...
/*
 * Load a matrix directly, bypassing the normal matrix stack path.
 *
 * mptr = pointer to the source matrix
 */
...
/*
 * Overwrite a single field of an already-loaded vertex.
 *
 * point = vertex slot number, 0-15
 * where = byte offset of the field within the vertex
 * num   = replacement 32-bit value
 */
...
/*
 * n must be one of NUMLIGHTS_0 through NUMLIGHTS_7. There is always one
 * additional ambient light beyond the directional count given here.
 */
...
#define LIGHT_8		8
/*
 * l points to a Light struct; n is one of LIGHT_1 through LIGHT_8.
 * Whatever the active light count is, the highest-numbered slot is
 * always the ambient light — e.g. with gsSPNumLights(NUMLIGHTS_3),
 * LIGHT_1 through LIGHT_3 are directional and LIGHT_4 is ambient.
 */
...
/*
 * Updates a light's color without touching its direction.
 * col packs r,g,b,a into one 32-bit word (alpha is ignored).
 * n is one of LIGHT_1 through LIGHT_8.
 */
...
/* The "name" argument below refers to a lights structure initialized via one of the gdSPDefLights macros. */
 ...
 * FOG macros
 *
 * gSPFogFactor takes fm (a z multiplier) and fo (a z offset) directly:
 * fog alpha = (eyespace z) * fm + fo, clamped to 0-255, where eyespace z
 * ranges from -1 to 1.
 *
 * gSPFogPosition is an alternate way to express the same thing using a
 * min/max range from 0 to 1000, where 0 is the near plane and 1000 is
 * the far plane. min is where fog starts fading in (often 0) and max is
 * where it reaches full thickness (often 1000).
 */
...
/*
 * Variant of gSPTexture with an extra parameter that the microcode
 * currently leaves reserved.
 */
...
/*
 * 'blendmask' is no longer a real mode; its bits are reserved and this
 * macro is kept only as a no-op for source compatibility.
 */
...
/*
 * gDPSetCombineMode's indirection through gDPSetCombineLERP is
 * deliberate, not redundant: it lets the preprocessor substitute a
 * single macro argument that itself expands to a comma-separated list,
 * then rescan the result against the longer-argument LERP macro.
 *
 * e.g. gsDPSetCombineMode(G_CC_MODULATE, G_CC_MODULATE) expands to
...
#endif
/*
 * Dxt is the reciprocal of the number of 64-bit words per line of the
 * texture being loaded via load_block, expressed as a 1.11 fixed-point
 * value. It needs to round up whenever the true value has any bits set
 * past the 11th fractional bit; the macros below handle that rounding.
 * The 4-bit variants are separate because 4-bit textures are loaded as
 * if they were 8-bit.
 */
#define	G_TX_DXT_FRAC	11

/*
 * On RCP 2.0, a single load_block command can move at most 2048 texels.
 * To load the full 4kB of Tmem despite that cap, the load has to use
 * G_IM_SIZ_16b for the transfer itself and then have the tile's texel
 * size corrected afterward. The g*DPLoadTextureBlock macros already
 * handle this swap internally; calling g*DPLoadBlock directly means
 * doing that tile-size correction by hand.
 */
#ifdef _HW_VERSION_1
...
/* The trailing S means odd lines have already been word-swapped. */
...
/*
 * Allows the tmem address and render tile to be specified. The trailing
 * S means odd lines have already been word-swapped.
 */
...
/*
 * Static-initializer equivalent of gDPLoadTextureBlockS(): same
 * pre-swapped loading behavior, but with Dxt left at 0 instead of
 * computed.
 */
...
/*
 * Allows tmem and render tile to be specified, for loading several tiles
 * at once. This is the static-initializer equivalent of the pre-swapped
 * loading in gDPLoadTextureBlockS(), with Dxt left at 0 instead of
 * computed.
 */
...
/*
 * Textured rectangles take up 128 bits, twice the normal command size,
 * so gsDPTextureRectangle() generally isn't appropriate for use inside a
 * display list — use gsSPTextureRectangle() there instead. This is also
 * why no gDPTextureRectangle() macro exists.
 */
