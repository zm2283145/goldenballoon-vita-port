...
/*
 * gu.h — clean-room graphics utility declarations.
 *
 * Declares the graphics utility math layer built on top of the binary
 * interfaces from mbi.h: matrix construction and projection helpers
 * (identity, ortho, frustum, perspective, look-at, rotate/scale/translate),
 * positional-light utilities, and the display-list dump/parse routines used
 * for debugging. This is a declaration-only compatibility surface for this
 * port; no vendor implementation is present.
 */
 ...
 * matrix operations:
 *
 * Each of these has an 'F' counterpart that works entirely in floating
 * point, for callers that want to do their matrix math in float and only
 * convert to the fixed-point Mtx format as a final step.
 */
...
#define GU_PARSERDP_DUMPONLY           32  /* value need not match */
                                           /* GU_PARSEGBI_DUMPONLY -- it's */
                                           /* only shared so callers can reuse it */

extern void guParseRdpDL(u64 *rdp_dl, u64 nbytes, u8 flags);
extern void guParseString(char *StringPointer, u64 nbytes);

/*
 * Retired — call guParseRdpDL with the GU_PARSERDP_DUMPONLY flag instead.
 */
