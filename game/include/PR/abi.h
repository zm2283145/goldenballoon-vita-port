...
/*
 * abi.h — clean-room audio binary interface declarations.
 *
 * Declares the Acmd audio command packet layouts and the aXxx macros that
 * assemble them into a command list consumed by the RSP audio microcode.
 * It is a declaration-only compatibility surface for this port — no vendor
 * implementation is present — and is pulled in via mbi.h alongside the
 * graphics binary interface, whose packet-building conventions it mirrors.
 */
...
#ifdef NATIVE_PORT
/* mdkr64 (M5 audio): pull in the software aspMain mixer LAST so its #undef +
 * redefinition of the a* packet-builder macros above wins in every audio-synth
 * TU that includes abi.h (via libaudio.h -> mbi.h), regardless of that TU's own
 * include order. "Building the Acmd list" then IS "running the synthesis" —
 * the macros call directly into platform/mixer.c. See platform/mixer.h. */
#include "mixer.h"
#endif
