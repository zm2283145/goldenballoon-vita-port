...
/*
 * os_pi.h — clean-room peripheral-interface DMA declarations.
 *
 * Declares the queued DMA request format and per-domain handle used to
 * move data between RDRAM and cartridge-domain devices over the
 * peripheral interface, including the enhanced-PI calls for probing
 * and driving alternate domains. This is a declaration-only
...
/*
 * Tracks an in-progress disk-DMA transfer so the exception handler can
 * keep it updated; this is the shared state the PI manager and the
 * interrupt handler use to hand off status to each other.
 */
