...
/*
 * region.h — clean-room fixed-buffer memory region allocator declarations.
 *
 * Declares the OSRegion header structure and the alignment/size constants
 * that describe a pool of same-sized buffers, along with the allocate,
 * free, and query entry points used to manage one. It is a
 * declaration-only compatibility surface: no vendor implementation is
 * present, only the layout and constants the rest of the port relies on.
 */
