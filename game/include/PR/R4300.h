/*
 * R4300.h — clean-room CPU register, status/cause bit and exception-vector
 * declarations for the MIPS core.
 *
 * Declares the segment base addresses, exception vector offsets, TLB and
 * cache-instruction constants, and the Status/Cause/Config coprocessor-0
 * register bit fields for the R4300-family CPU this port targets. It is a
 * declaration-only compatibility surface: no vendor implementation is
 * present, only the constants the rest of the port relies on.
 */
...
/* Hardware watchpoint registers (R4000-family feature):
 *   C0_WATCHLO:
 *     bits 31..3 hold bits 31..3 of the physical address to watch
 *     bit 2:  reserved, write as 0
 *     bit 1:  set to trap on loads from that address
 *     bit 0:  set to trap on stores to that address
 *   C0_WATCHHI
 *     bits 31..4 are reserved, write as 0
 *     bits 3..0 hold bits 35..32 of the physical address to watch
 */
