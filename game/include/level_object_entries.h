...
/*
 * Object-map records are encoded at two-byte granularity. Hud is the only
 * record type with a 32-bit member, so its natural C alignment would otherwise
 * raise the complete LevelObjectEntry union to four and make every record that
 * begins at 2 mod 4 undefined to access. Pack this serialized view to the
 * stream's real two-byte contract; its field offsets and 12-byte encoded size
 * stay unchanged.
 */
#if defined(NATIVE_PORT)
#pragma pack(push, 2)
...
_Static_assert(sizeof(LevelObjectEntryCommon) == 8,
               "LevelObjectEntryCommon must retain its encoded size");
_Static_assert(_Alignof(LevelObjectEntryCommon) == 2,
               "level-object records are encoded on two-byte boundaries");
_Static_assert(sizeof(LevelObjectEntry_Boost2) == 10,
               "dynamic boost entry must retain its encoded size");
_Static_assert(sizeof(LevelObjectEntry_Hud) == 12,
               "HUD entry must retain its encoded size");
_Static_assert(offsetof(LevelObjectEntry_Hud, offsetY) == 8,
               "HUD offsetY must retain its encoded offset");
_Static_assert(_Alignof(LevelObjectEntry_Hud) == 2,
               "HUD offsetY is a two-byte-aligned serialized s32");
_Static_assert(_Alignof(LevelObjectEntry) == 2,
               "LevelObjectEntry must be valid at every encoded record start");
#endif
