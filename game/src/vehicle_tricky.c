...
void update_tricky(s32 updateRate, f32 updateRateF, Object *obj, Object_Racer *racer, u32 *input, u32 *buttonsPressed,
...
#ifdef NATIVE_PORT
    /* TEST HOOK -- MDKR_BOSS_SLOW=1 cripples the boss so the human finishes first,
     * which is the only way to reach racer_boss_finish()'s finishPos == 1 arm from
     * a headless route (autopilot drives the human with the boss's own AI, so it
     * always comes second). See platform/stubs_dkr.c. No-op unless set. */
    {
        extern int mdkr_boss_slow(void);
        if (racer->playerIndex == PLAYER_COMPUTER && mdkr_boss_slow()) {
            racer->velocity *= 0.15f;
        }
    }
#endif
    *startTimer = tempStartTimer;
    ...
    diffX = (DKR_PTR(ObjectModel_44, objModel->animations)[obj->animationID].animLength * 16) - 17;
...
#ifdef NATIVE_PORT
    /* The boss-race VERDICT, traced at the one place it is read. Reported as
     * "I was awarded first place, still got the failure animation and was
     * returned without having won", i.e. the HUD position and the boss state
     * machine disagreeing -- so the thing to make observable is the exact value
     * this function branches on, next to the course/world state it indexes with.
     * tests/check_collision_gridmask.py asserts on finishPos in both directions.
     * Trace-gated; one line per boss race. */
    {
        mdkr_trace("bossfinish: finishPos=%d playerIndex=%d courseId=%d worldId=%d bosses=0x%x courseFlags=0x%x "
                   "cutsceneLevel=%d timer=%d raceFinished=%d",
                   (int) finishPos, (int) racer->playerIndex, (int) settings->courseId, (int) settings->worldId,
                   (unsigned) settings->bosses, (unsigned) settings->courseFlagsPtr[settings->courseId], (int) i,
                   (int) arg1_ret, (int) racer->raceFinished);
    }
#endif
    if (arg1_ret == 1) {
        if (finishPos == 1) {
