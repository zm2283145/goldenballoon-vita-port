...
LensFlareData gLensPresetLarge[2] = { { 1, { { { 255, 255, 192, 144 } } }, 3.0f, 0.0f }, { 0 } };
...
void weather_reset(s32 weatherType, s32 density, s32 velX, s32 velY, s32 velZ, s32 intensity, s32 opacity) {
    ...
    for (i = 0; i < gSnowDensity; i++) {
        ...
        gSnowPhysics[i].unused_C = DKR_SHL32(1U, rand_range(0, 32) + 5);
        gSnowPhysics[i].unused_D = DKR_SHL32(1U, rand_range(0, 32) + 5);
        gSnowPhysics[i].unused_E = DKR_SHL32(1U, rand_range(0, 32) + 5);
...
void snow_init(void) {
    ...
    for (i = 0; i < gSnowGfx.size; i++) {
        gSnowGfx.pos[i].x = coss_s16(offset & 0xFFFF) * 8;
        gSnowGfx.pos[i].y = 0xFFFC0000;
        gSnowGfx.pos[i].z = sins_s16(offset & 0xFFFF) * 2;
        offset += step;
    }
...
#endif
        /* gSnowPlane.current STAYS render-side: it is derived from
         * gSnowPlane.near/far, which weather_clip_planes() sets per VIEWPORT
         * immediately before this call, so it is a property of the draw rather
         * than of the tick. The integrators it reads (gWeather.opacity) are the
         * tick's. */
        gSnowPlane.current =
...
#ifdef NATIVE_PORT
        /* Colour-only. set_fog() also clears
         * gFogData[0].switchTimer and gFogData[0].fogChanger, which re-armed
         * every fog-changer object on the track once per frame and cancelled
         * any slowly_change_fog() fade in progress. See set_fog_colour(). */
        set_fog_colour(0, a, b, 28, 15, 36);
#else
 ...
 *
 * NATIVE_PORT compiles only the billboard emission here. Everything that
 * integrates state or consumes RNG -- the splash spawn, the slot allocation and
 * the animFrame advance -- belongs to rain_splash_tick() below, which runs on
 * the tick's clock rather than once per viewport. The spawn path must exist in
 * exactly one of the two, or its rand_range() rolls are consumed twice.
 */
void rain_render_splashes(UNUSED s32 updateRate) {
    s32 i;
    s32 setEnvColor;
#ifndef NATIVE_PORT
    s32 temp_t0;
...
#ifdef NATIVE_PORT
                    render_sprite_billboard_transform(
                        &gCurrWeatherDisplayList, &gCurrWeatherMatrix,
                        &gCurrWeatherVertexList, &gRainSplashSegments[i].trans,
                        gRainSplashSegments[i].animFrame, gRainSplashGfx,
                        RENDER_Z_COMPARE | RENDER_SEMI_TRANSPARENT |
                            RENDER_FOG_ACTIVE | RENDER_Z_UPDATE);
#else
                    render_sprite_billboard(
                        &gCurrWeatherDisplayList, &gCurrWeatherMatrix,
                        &gCurrWeatherVertexList, (Object *) &gRainSplashSegments[i],
                        gRainSplashGfx,
                        RENDER_Z_COMPARE | RENDER_SEMI_TRANSPARENT |
                            RENDER_FOG_ACTIVE | RENDER_Z_UPDATE);
#endif
 ...
 *
 * Everything here integrates or consumes RNG: gRainSplashDelay, the two
 * rand_range() rolls that place a splash and the third that picks its opacity,
 * the gRainSplashSegments slot allocation, and the animFrame advance that
 * retires a splash. rain_render_splashes() keeps only the billboard emission.
 */
static void rain_splash_tick(s32 updateRate) {
    s32 temp_t0;
    s32 i;
    f32 var_f2;
    s32 firstIndexWithoutFlags;
        ...
        if (gRainSplashDelay <= 0) {
            ...
            if (racer != NULL) {
                ...
                if (firstIndexWithoutFlags >= 0) {
                    ...
                    xPos = (sins_f(randYRot) * randFloat) + racer->trans.x_position;
                    zPos = (coss_f(randYRot) * randFloat) + racer->trans.z_position;
                    /* This query is for a random point, not for anywhere the
                     * simulation cares about, so it would clobber the single
                     * global wave cache that get_wave_properties() reads. Bracket
                     * it; the full rationale is on wave_query_cache_save() in
                     * tracks.c. */
                    wave_query_cache_save(&savedWaveQuery);
                    i = get_level_segment_waves(
                        get_level_segment_index_from_position(xPos, racer->trans.y_position, zPos), xPos, zPos,
                        &waterProps);
                    if (i != 0) {
...
/**
 * Every weather integrator, hoisted out of render_scene's per-viewport
 * weather_update() call.
 *
 * What made this authoritative rather than cosmetic:
 *
 *   - weather.c's rand_range() calls (splash placement and opacity, and the
 *     lightning interval) mutate gCurrentRNGSeed, which racer AI consumes
 *     (racer.c:338/4335/4539/5219/5223/5415/5872/9019). Because these rolls now
 *     happen on the tick's clock they are AUTHORITATIVE and deliberately
 *     UNBRACKETED -- weather randomness is real gameplay randomness, consumed
 *     once per tick. The save_rng_seed/load_rng_seed bracket around
 *     weather_update() in render stays, and now covers only whatever residual
 *     render-side work remains.
 *   - rain_lightning() calls transition_begin(&gThunderTransition), and
 *     check_fadeout_transition() gates objects.c:8482/11226 and
 *     vehicle_tricky.c:219 -- so a screen transition, and the object behaviour
 *     that keys off it, was started from inside the draw.
 *
 * Vertex emission, the rain overlay, the splash billboards, gSnowVertexFlip and
 * gRainVertexFlip all stay render-side: they are per-draw and the flips are
 * not idempotent.
 */
void weather_tick(s32 updateRate) {
    UNUSED s32 unused;
