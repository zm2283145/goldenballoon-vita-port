...
void populate_settings_from_save_data(Settings *settings, u8 *saveData) {
    ...
    if (var_a0 == 0) {
...
#ifdef NATIVE_PORT
                settings->courseFlagsPtr[i] |=
                    (s32) mdkr_save_course_status_to_flags(temp_v1);
#else
                if (temp_v1 > 0) {
                    // Set Map Visited
        ...
        func_80072C54(8);
#ifdef NATIVE_PORT
        /* NATIVE_PORT: a BEATEN Taj challenge implies an OFFERED Taj challenge.
         *
         * tajFlags packs two sets of three: 0x01/0x02/0x04 = car/hover/plane
         * challenge OFFERED (set by track_setup_racers() the first time Taj
         * teleports over to propose it), 0x08/0x10/0x20 = the same three BEATEN
         * (set by mode_end_taj_race() on a first-place finish). Those are two
         * separate writes, minutes apart, each with its own
         * safe_mark_write_save_file() and — in the browser — its own IDBFS sync.
         *
         * If only the later one lands, the save says "beaten but never offered",
         * and that state is a trap: the offer gate at objects.c:1794 tests ONLY
         * the OFFERED bits, so Taj teleports over again on entry to Timber's
         * Island, and the auto-offer dialogue chain does not ask — DIALOGUEPAGE
         * _TAJ_CHALLENGE_CAR falls straight through to DIALOGUEPAGE_TAJ_4, which
         * returns `(gNextTajChallengeMenu - 1) | 0x40` and starts the race. The
         * player is put into a challenge they have already beaten, which is
         * exactly what was reported from the published build. Measured on us.v80:
         * tajFlags 0x08 with 5 balloons re-runs the unlock branch with j=1.
         *
         * The implication is sound in one direction only, which is why this is a
         * repair and not a guess: a challenge cannot be beaten without having
         * been offered, so OR-ing OFFERED in from BEATEN can only ever restore a
         * bit that play must already have set. It fabricates no progress (it
         * never sets a BEATEN bit), it is idempotent, and it is a no-op on every
         * consistent save — verified for 0x00/0x01/0x03/0x07/0x09/0x1B/0x1F/0x3F.
         *
         * The write-side cause is fixed separately (the browser shell and the
         * engine now share one FS.syncfs coalescer instead of two independent
         * ones — see docs/OPEN_ITEMS.md "savefailsafe"). This is the belt to that
         * braces: even if a progress write is lost, the symptom cannot come back,
         * because the loop needs the two halves to disagree.
         */
        settings->tajFlags |= (settings->tajFlags >> 3) & TAJ_FLAGS_UNLOCKED_A_CHALLENGE;

        /* NATIVE_PORT: reject a slot whose 16-bit checksum passes but whose
         * fields cannot have come from play.
         *
         * The amulets have FOUR pieces each. The game's own completion tests are
         * `settings->wizpigAmulet >= 4` (game.c) and `settings->ttAmulet >= 4`
         * (game.c / object_functions.c), and vehicle_tricky.c uses
         * `settings->wizpigAmulet - 1` as a 0..3 cutscene index — so 5..7 are
         * unreachable in a real save. On disk the field is 3 bits, so 5..7 are
         * perfectly representable, and the slot checksum is a plain byte sum
         * that a corrupt image can satisfy by chance.
         *
         * That matters because objects.c's BHV_DYNAMIC_LIGHT_OBJECT_2 spawn does
         * `assetCount = settings->wizpigAmulet + 1; modelIndex = wizpigAmulet`
         * with no clamp against the object's numberOfModelIds. Measured on
         * us.v80: a checksum-valid slot with wizpigAmulet 5 or 7 loads
         * out-of-range model ids for that object on Timber's Island and then
         * SIGSEGVs inside the racer collision path (func_80054FD0, reached from
         * update_player_racer) about 750 frames after the hub loads. In the
         * browser build the save lives in IndexedDB, so that is a PERMANENT boot
         * loop: crash, reload, same save, crash. 4 of 40 randomly-corrupted
         * checksum-valid images in the fuzz behind tests/check_save_failsafe.py
         * hit it.
         *
         * Fail toward a clean boot using the function's own idiom for "this slot
         * is not a save": leave newGame TRUE. read_save_file() then calls
         * erase_save_file(), which rewrites the slot to 0xFF, and FILE SELECT
         * shows an unstarted file instead of crashing on entry. The original
         * bytes are preserved by the quarantine in platform/stubs_dkr.c.
         *
         * Scope is deliberately narrow: this is the one field the fuzz proved
         * fatal (every other field at its maximum representable value survives
         * the same route). The deeper fix — clamping modelIndex against
         * numberOfModelIds in objects.c — belongs with the object code.
         */
        if (settings->ttAmulet > 4 || settings->wizpigAmulet > 4) {
            clear_game_progress(settings);
        } else {
...
#ifdef NATIVE_PORT
            /*
             * Persist the longest valid prerequisite prefix. The old additive
             * encoder mapped impossible combinations to unrelated states
             * (CLEARED alone became VISITED; VISITED|SILVER became CLEARED),
             * fabricating one kind of progress while discarding another.
             */
            courseStatus = mdkr_save_course_flags_to_status(
                (u32) settings->courseFlagsPtr[i]);
#else
            courseStatus = 0;
            // Map visited
...
#define BLANK_EXT_CHAR ' '
    ...
    u32 occupiedExtensions;
    s32 fileNum;
    char fileExtChar;
    SIDeviceStatus ret;

    fileExtChar = BLANK_EXT_CHAR;
    occupiedExtensions = 0;
    ret =
    ...
    if (ret == CONTROLLER_PAK_GOOD) {
        for (fileNum = 0; fileNum < MAX_CPAK_FILES; fileNum++) {
...

            {
                ...
                if (fileExtensions[fileNum] == NULL) {
                    continue;
                }
                extension = fileExtensions[fileNum][0];
                if (!mdkr_extension_bit(extension, &extensionBit)) {
                    continue;
                }
                if (fileExtChar < extension) {
                    fileExtChar = extension;
                }
                occupiedExtensions |= extensionBit;
            }
        }
        if (fileExtChar == BLANK_EXT_CHAR) {
            fileExtChar = 'A';
        } else if (fileExtChar == 'Z') {
            for (fileExtChar = 'A'; fileExtChar <= 'Z';
                 occupiedExtensions >>= 1, fileExtChar++) {
...
#ifdef NATIVE_PORT
            /*
             * A save slot is one independently-checksummed 40-byte record.
             * Persisting its five EEPROM blocks separately exposes four
             * checksum-invalid intermediate images to IDBFS. A refresh during
             * one of those writes can therefore turn a valid Adventure file
             * into a corrupt slot that recovery must erase. Commit the complete
             * logical record in one platform transaction.
             */
            if (osEepromLongWrite(si_mesg(), startingAddress, saveData,
                                  blockSize * (s32) sizeof(u64)) != 0) {
                stubbed_printf("WARNING : Could not erase save file\n");
            }
#else
...
#ifdef NATIVE_PORT
        /*
         * The checksum covers the complete five-block slot. Keep the file and
         * browser backing store on either the old valid slot or the new valid
         * slot; never publish a partially updated checksum domain.
         */
        if (osEepromLongWrite(si_mesg(), startingAddress, (u8 *) alloc,
                              blocks * (s32) sizeof(u64)) != 0) {
            mempool_free(alloc);
            return -1;
        }
#else
...
#ifdef NATIVE_PORT
            if (osEepromLongWrite(
                    si_mesg(), BLOCK_SIZE(FASTEST_LAPS_START),
                    (u8 *) alloc, blocks * (s32) sizeof(u64)) != 0) {
                mempool_free(alloc);
                return -1;
            }
#else
...
#ifdef NATIVE_PORT
            if (osEepromLongWrite(
                    si_mesg(), BLOCK_SIZE(COURSE_TIMES_START),
                    (u8 *) (&alloc[sizeof(CourseRecords) / sizeof(u64)]),
                    blocks * (s32) sizeof(u64)) != 0) {
                mempool_free(alloc);
                return -1;
            }
...
s32 read_eeprom_settings(u64 *eepromSettings) {
    ...
    s32 expected;
#ifdef NATIVE_PORT
    u8 serializedSettings[sizeof(*eepromSettings)];
#endif
...
#ifdef NATIVE_PORT
    /*
     * EEPROM is an N64-owned, big-endian serialized format. The original game
     * could read its eight bytes straight into a u64 because the console CPU is
     * big-endian; doing that on little-endian native/web hosts reverses every
     * option and unlock bit (including Adventure Two) and then rejects the
     * otherwise-valid checksum. Keep osEepromRead() byte-oriented and convert
     * only at this typed serialization boundary.
     */
    osEepromRead(si_mesg(), BLOCK_SIZE(CONFIG_START), serializedSettings);
    *eepromSettings = mdkr_read_be_u64(serializedSettings);
#else
...
s32 write_eeprom_settings(u64 *eepromSettings) {
    ...
    *eepromSettings |= (u64) (calculate_eeprom_settings_checksum(*eepromSettings)) << 56;
    if (is_reset_pressed() == FALSE) {
#ifdef NATIVE_PORT
        mdkr_write_be_u64(serializedSettings, *eepromSettings);
        osEepromWrite(si_mesg(), BLOCK_SIZE(CONFIG_START), serializedSettings);
#else
...
#define DKR_GHOST_SLOT_COUNT 6
#define DKR_GHOST_DIRECTORY_ENTRIES (DKR_GHOST_SLOT_COUNT + 1)
#define DKR_GHOST_MAX_NODES 360

static s16 calculate_ghost_header_checksum_bounded(
    const GhostHeader *ghostHeader, size_t availableBytes, s32 *validOut) {
    size_t length;
    size_t i;
    u16 sum = 0;
    s32 nodeCount;
    ...
    if (ghostHeader == NULL || availableBytes < sizeof(*ghostHeader)) {
        return 0;
    }
    nodeCount = ghostHeader->nodeCount;
    if (nodeCount < 0 || nodeCount > DKR_GHOST_MAX_NODES ||
        (size_t)nodeCount >
            (SIZE_MAX - sizeof(*ghostHeader)) / sizeof(GhostNode)) {
        return 0;
    }
    length = sizeof(*ghostHeader) + (size_t)nodeCount * sizeof(GhostNode);
    if (length > availableBytes) {
        return 0;
    }
    for (i = 2; i < length; i++) {
        sum = (u16)(sum + ((const u8 *)ghostHeader)[i]);
    }
    if (validOut != NULL) {
        *validOut = TRUE;
    }
...
s16 calculate_ghost_header_checksum(GhostHeader *ghostHeader) {
    s32 valid;
    size_t maximumBytes =
        sizeof(*ghostHeader) + DKR_GHOST_MAX_NODES * sizeof(GhostNode);
    s16 checksum = calculate_ghost_header_checksum_bounded(
        ghostHeader, maximumBytes, &valid);
    return valid ? checksum : 0;
}

static s32 ghost_record_is_valid(
    const GhostHeader *ghostHeader, size_t availableBytes) {
    s32 valid;
    s16 checksum;
    if (ghostHeader == NULL) {
        return FALSE;
    }
    checksum = ghostHeader->checksum;
    return calculate_ghost_header_checksum_bounded(
               ghostHeader, availableBytes, &valid) == checksum &&
           valid;
}

/*
 * Validate the seven directory offsets before any subtraction, allocation, or
 * pointer formation. `bytes` only needs to contain the 0x100-byte directory;
 * `fileSize` is the full note size used to constrain its payload extents.
 */
static s32 ghost_directory_is_valid(
    const unk80075000 *file, size_t bytes, size_t fileSize) {
    const unk80075000_body *directory;
    s32 previous = GHSS_SIZE;
    s32 i;
    if (file == NULL || bytes < GHSS_SIZE || fileSize < GHSS_SIZE ||
        fileSize > INT16_MAX || file->signature != GHSS) {
        return FALSE;
    }
    directory = file->data;
    for (i = 0; i < DKR_GHOST_DIRECTORY_ENTRIES; i++) {
        s32 offset = directory[i].unk2;
        if (offset < GHSS_SIZE || offset < previous ||
            (size_t)offset > fileSize) {
            return FALSE;
        }
        if (i < DKR_GHOST_SLOT_COUNT && directory[i].unk0 != 0xFF) {
            s32 next = directory[i + 1].unk2;
            if (directory[i].unk1 > 2 || next < offset ||
                (size_t)(next - offset) < sizeof(GhostHeader)) {
                return FALSE;
            }
        }
        previous = offset;
...
static s32 ghost_file_is_valid(
    const unk80075000 *file, size_t fileSize) {
    const unk80075000_body *directory;
    s32 i;
    if (!ghost_directory_is_valid(file, fileSize, fileSize)) {
        return FALSE;
    }
    directory = file->data;
    for (i = 0; i < DKR_GHOST_SLOT_COUNT; i++) {
        if (directory[i].unk0 != 0xFF) {
            s32 offset = directory[i].unk2;
            s32 next = directory[i + 1].unk2;
            if (!ghost_record_is_valid(
                    (const GhostHeader *)(AS_BYTES(file) + offset),
                    (size_t)(next - offset))) {
                return FALSE;
            }
        }
...
s32 func_80074AA8(
    GhostHeader *ghostHeader, s16 characterID, s16 time, s16 nodeCount,
    u8 *dest) {
    if (ghostHeader == NULL ||
        !ghost_write_args_are_valid(
            0, 0, characterID, nodeCount, dest)) {
        return FALSE;
    }
    ...
    if (nodeCount != 0) {
        bcopy(
            dest, (u8 *)ghostHeader + sizeof(*ghostHeader),
            (size_t)nodeCount * sizeof(GhostNode));
    }
...
s32 func_80074B34(s32 controllerIndex, s16 levelId, s16 vehicleId, u16 *ghostCharacterId, s16 *ghostTime,
    ...
    if (ghostCharacterId != NULL) {
        if (ghostTime == NULL || ghostNodeCount == NULL || ghostData == NULL) {
            start_reading_controller_data(controllerIndex);
    ...
    if (pakStatus == CONTROLLER_PAK_GOOD) {
        ...
        if (pakStatus != CONTROLLER_PAK_GOOD || fileSize < GHSS_SIZE) {
            start_reading_controller_data(controllerIndex);
            return CONTROLLER_PAK_BAD_DATA;
        }
        ...
        if (pakStatus == CONTROLLER_PAK_GOOD) {
            if (ghost_directory_is_valid(
                    cPakFile, GHSS_SIZE, (size_t)fileSize)) {
        ...
        if (ghostSize != 0) {
            if (ghostCharacterId != NULL) {
                cPakFile = mempool_alloc_safe(allocateSpace, COLOUR_TAG_BLACK);
                if (osPfsReadWriteFile(&pfs[controllerIndex], fileNumber, PFS_READ, ghostSize, allocateSpace,
                                       AS_BYTES(cPakFile)) == 0) {
                    GhostHeader *header = (GhostHeader *)cPakFile;
                    if (ghost_record_is_valid(header, (size_t)allocateSpace)) {
                        *ghostCharacterId = header->characterID;
                        *ghostTime = header->time;
                        *ghostNodeCount = header->nodeCount;
                        bcopy(
                            AS_BYTES(cPakFile) + sizeof(*header), ghostData,
                            (size_t)*ghostNodeCount * sizeof(GhostNode));
...
SIDeviceStatus func_80074EB8(s32 controllerIndex, s16 levelId, s16 vehicleId, s16 ghostCharacterId, s16 ghostTime,
    ...
    if (!ghost_write_args_are_valid(
            levelId, vehicleId, ghostCharacterId, ghostNodeCount, dest)) {
        return CONTROLLER_PAK_BAD_DATA;
    }
    if (1) {} // fake
    sp30 += GHSS_SIZE;
    sp24 = sp30 * 6;
    if (1) {} // fake
    ghost = mempool_alloc_safe(sp24 + GHSS_SIZE, COLOUR_TAG_BLACK);
    bzero(ghost, sp24 + GHSS_SIZE);
    ...
    if (!func_80074AA8(
            (GhostHeader *)(AS_BYTES(ghost) + ghostBody[0].unk2),
            ghostCharacterId, ghostTime, ghostNodeCount, dest)) {
        mempool_free(ghost);
        return CONTROLLER_PAK_BAD_DATA;
    }
    if (!ghost_file_is_valid(ghost, (size_t)(sp24 + GHSS_SIZE))) {
        mempool_free(ghost);
        return CONTROLLER_PAK_BAD_DATA;
    }
    pakStatus = write_controller_pak_file(
        controllerIndex, -1, "DKRACING-GHOSTS", "", (u8 *)ghost,
        sp24 + GHSS_SIZE);
    mempool_free(ghost);
    return pakStatus;
}
...
SIDeviceStatus func_80075000(s32 controllerIndex, s16 levelId, s16 vehicleId, s16 ghostCharacterId, s16 ghostTime,
    ...
    if (ghostData == NULL ||
        !ghost_write_args_are_valid(
            levelId, vehicleId, ghostCharacterId, ghostNodeCount,
            &ghostData->unk0)) {
        return CONTROLLER_PAK_BAD_DATA;
    }
    ...
    if (pakStatus != CONTROLLER_PAK_GOOD) {
        ...
        if (pakStatus != CONTROLLER_PAK_GOOD) {
            ...
            if (fileSize < GHSS_SIZE) {
                start_reading_controller_data(controllerIndex);
                return CONTROLLER_PAK_BAD_DATA;
            }
            fileData = mempool_alloc_safe(fileSize, COLOUR_TAG_BLACK);
            pakStatus = read_data_from_controller_pak(controllerIndex, fileNumber, AS_BYTES(fileData), fileSize);
            start_reading_controller_data(controllerIndex);
            if (pakStatus != CONTROLLER_PAK_GOOD) {
                mempool_free(fileData);
            } else {
                if (ghost_file_is_valid(fileData, (size_t)fileSize)) {
                    ghostFileData = &fileData->data[0];

                    for (i = 0, ghostIndex = -1;
                         i < DKR_GHOST_SLOT_COUNT; i++) {
                        if (levelId == ghostFileData[i].unk0 &&
                            vehicleId == ghostFileData[i].unk1) {
                            ghostIndex = i;
                            break;
                        }
                    }
                    if (ghostIndex != -1) {
                        GhostHeader *existing = (GhostHeader *)(
                            AS_BYTES(fileData) +
                            ghostFileData[ghostIndex].unk2);
                        if (existing->time < ghostTime) {
                            ghostIndex = -2;
                        }
                    }
                    ...
                    if (ghostIndex == -2) {
                        ...
                        if ((s32)ghostFileData[DKR_GHOST_SLOT_COUNT].unk2 +
                                sp58 >
                            fileSize) {
                            pakStatus = CONTROLLER_PAK_NO_ROOM_FOR_GHOSTS;
                            mempool_free(fileData);
                        ...
                        fileDataToWrite =
                            mempool_alloc_safe(fileSize, COLOUR_TAG_BLACK);
                        bzero(fileDataToWrite, fileSize);
                        fileDataToWrite->signature = GHSS;
                        sp70 = &fileDataToWrite->data[0];
                        fileDataToWrite->data[DKR_GHOST_SLOT_COUNT].unk0 =
                            0xFF;
                        fileDataToWrite->data[DKR_GHOST_SLOT_COUNT].unk2 =
                            (s16)(
                                ghostFileData[DKR_GHOST_SLOT_COUNT].unk2 +
                                sp58);
                        for (i = 0; i < DKR_GHOST_SLOT_COUNT; i++) {
                            sp70[i].unk0 = ghostFileData[i].unk0;
                            sp70[i].unk1 = ghostFileData[i].unk1;
                            if (ghostIndex >= i) {
                                sp70[i].unk2 = ghostFileData[i].unk2;
                            } else {
                                sp70[i].unk2 =
                                    (ghostFileData[i].unk2 + sp58);
                            }
                            if (i != ghostIndex) {
                                bcopy(
                                    AS_BYTES(fileData) +
                                        ghostFileData[i].unk2,
                                    AS_BYTES(fileDataToWrite) + sp70[i].unk2,
                                    ghostFileData[i + 1].unk2 -
                                        ghostFileData[i].unk2);
                            }
                        }
                        if (!func_80074AA8(
                                (GhostHeader *)(
                                    AS_BYTES(fileDataToWrite) +
                                    sp70[ghostIndex].unk2),
                                ghostCharacterId, ghostTime, ghostNodeCount,
                                &ghostData->unk0)) {
                        ...
                        if (!ghost_file_is_valid(
                                fileDataToWrite, (size_t)fileSize)) {
                            pakStatus = CONTROLLER_PAK_BAD_DATA;
                        } else {
                            pakStatus = write_controller_pak_file(
                                controllerIndex, fileNumber,
                                "DKRACING-GHOSTS", "",
...
s32 func_800753D8(s32 controllerIndex, s32 worldId) {
    ...
    data2 = NULL;
    /* Despite the decomp name, this is a directory slot selected from
     * gGhostWorldIDs[], not a World enum. Slot zero is the first saved ghost
     * and must be erasable; using WORLD_DINO_DOMAIN (1) rejected it. */
    if (worldId < 0 || worldId >= DKR_GHOST_SLOT_COUNT) {
        return CONTROLLER_PAK_BAD_DATA;
    }
    pakStatus = get_si_device_status(controllerIndex);
    if (pakStatus != CONTROLLER_PAK_GOOD) {
    ...
    if (pakStatus == CONTROLLER_PAK_GOOD) {
        ...
        data = mempool_alloc_safe(fileSize, COLOUR_TAG_BLACK);
        pakStatus = read_data_from_controller_pak(controllerIndex, fileNumber, (u8 *) data, fileSize);
        start_reading_controller_data(controllerIndex);
        if (pakStatus == CONTROLLER_PAK_GOOD) {
            if (ghost_file_is_valid(data, (size_t)fileSize)) {
                tempData = data->data;
                if (tempData[worldId].unk0 == 0xFF) {
                    pakStatus = CONTROLLER_PAK_BAD_DATA;
                    mempool_free(data);
                    return pakStatus;
                }
                sizeDiff = tempData[worldId].unk2 - tempData[worldId + 1].unk2;
                data2 = mempool_alloc_safe(fileSize, COLOUR_TAG_BLACK);
                bzero(data2, fileSize);
                bcopy(data, data2, tempData[worldId].unk2); // Copy data into data2

                if (worldId != DKR_GHOST_SLOT_COUNT - 1) {
                    bcopy(AS_BYTES(data) + tempData[worldId + 1].unk2,
                          AS_BYTES(data2) + tempData[worldId + 1].unk2 + sizeDiff,
                          tempData[DKR_GHOST_SLOT_COUNT].unk2 -
                              tempData[worldId + 1].unk2);
                }
                tempData = data2->data;
                for (i = worldId; i < DKR_GHOST_SLOT_COUNT - 1; i++) {
                    tempData[i + 0].unk0 = tempData[i + 1].unk0;
                    tempData[i + 0].unk1 = tempData[i + 1].unk1;
                    tempData[i + 0].unk2 = tempData[i + 1].unk2 + sizeDiff;
                }
                tempData[DKR_GHOST_SLOT_COUNT - 1].unk0 = 0xFF;
                tempData[DKR_GHOST_SLOT_COUNT - 1].unk1 = 0;
                tempData[DKR_GHOST_SLOT_COUNT - 1].unk2 =
                    tempData[DKR_GHOST_SLOT_COUNT].unk2 + sizeDiff;
                tempData[DKR_GHOST_SLOT_COUNT].unk0 = 0xFF;
                tempData[DKR_GHOST_SLOT_COUNT].unk1 = 0;
                tempData[DKR_GHOST_SLOT_COUNT].unk2 =
                    tempData[DKR_GHOST_SLOT_COUNT - 1].unk2;
                if (!ghost_file_is_valid(data2, (size_t)fileSize)) {
                    pakStatus = CONTROLLER_PAK_BAD_DATA;
                } else {
                    pakStatus = write_controller_pak_file(
                        controllerIndex, fileNumber, "DKRACING-GHOSTS", "",
                        (u8 *)data2, fileSize);
                }
            } else {
                pakStatus = CONTROLLER_PAK_BAD_DATA;
...
SIDeviceStatus func_800756D4(
    ...
    u16 *checksumIDs, s32 outputCapacity) {
    s32 i;
    u8 *fileData;
    s32 ret; // sp64
    unk80075000 *ghostFile;
    unk80075000_body *directory;
    s32 fileNumber;
    s32 fileSize;
    u8 temp_v0_2;

    if (levelIDs == NULL || vehicleIDs == NULL || characterIDs == NULL ||
        checksumIDs == NULL || outputCapacity < DKR_GHOST_SLOT_COUNT) {
        return CONTROLLER_PAK_BAD_DATA;
    }
    ret = get_si_device_status(controllerIndex);
    if (ret != CONTROLLER_PAK_GOOD) {
    ...
    if (ret == CONTROLLER_PAK_GOOD) {
        ...
        if (ret == CONTROLLER_PAK_GOOD) {
            ...
            if (ret == CONTROLLER_PAK_GOOD) {
                ghostFile = (unk80075000 *)fileData;
                if (!ghost_directory_is_valid(
                        ghostFile, (size_t)fileSize, (size_t)fileSize)) {
                    ret = CONTROLLER_PAK_BAD_DATA;
                } else {
                    directory = ghostFile->data;
                    for (i = 0; i < DKR_GHOST_SLOT_COUNT; i++) {
                        if (directory[i].unk0 != 0xFF) {
                            s32 offset = directory[i].unk2;
                            s32 next = directory[i + 1].unk2;
                            GhostHeader *header =
                                (GhostHeader *)&fileData[offset];
                            if (!ghost_record_is_valid(
                                    header, (size_t)(next - offset))) {
                                ret = CONTROLLER_PAK_BAD_DATA;
                                break;
                            }
                            levelIDs[i] = directory[i].unk0;
                            vehicleIDs[i] = directory[i].unk1;
                            characterIDs[i] = header->characterID;
                            checksumIDs[i] = (u16)header->time;
                        }
                    }
                }
            }
...
SIDeviceStatus get_si_device_status(s32 controllerIndex) {
    ...
    /*
     * The native boundary exposes a composite virtual accessory: durable Pak
     * storage and host haptics can coexist even though original N64 hardware
     * required physically swapping them. Once PFS is initialized, prefer it
     * here; rumble_update() still probes osMotorInit directly and retains full
     * feedback. Without this priority, an empty SI queue could randomly make a
     * valid ghost Pak appear to have been replaced by a Rumble Pak.
     */
    if (sControllerMesgQueue->validCount == 0
#ifdef NATIVE_PORT
        && !(pfs[controllerIndex].status & PFS_INITIALIZED)
#endif
    ) {
...
char *string_to_font_codes(char *inString, char *outString, s32 stringLength) {
    ...
    /*
     * Controller Pak names are fixed-width 16-byte / 4-byte fields, not C
     * strings. Every caller hands us exactly that much storage. Writing a
     * courtesy terminator here therefore lands one byte past both arrays
     * (first reached at boot when a virtual Pak makes the CORE lookup live).
     * The remaining-width loop already zero-pads shorter input; a full-width
     * name intentionally has no terminator in the on-Pak representation.
     */
    return ret;
}
