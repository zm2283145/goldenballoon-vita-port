...
void lights_init(s32 count) {
    ...
    /* LP64: (u32) truncates the 64-bit arena pointer, leaving gShadeBuffer/
     * gLightDirs wild (they are dereferenced every frame the level has lights).
     * Carry the full host pointer through uintptr_t. */
    gShadeBuffer = (ObjectLightShadeProperties *) ((uintptr_t) gActiveLightAttrs + temp * sizeof(ObjectLight));
    gLightDirs = (Vec3f *) ((uintptr_t) gShadeBuffer + temp * sizeof(ObjectLightShadeProperties));
    ...
    if (gNumActiveLights < gMaxLights) {
        ...
        if (lightEntry->unk1C < 7) {
            ...
            if (((s32) levelHeader->unk74[lightEntry->unk1C]) != -1) {
                light->unk44_asset =
                    DKR_PTR(MiscAssetObjectHeader24, levelHeader->unk74[lightEntry->unk1C]);
