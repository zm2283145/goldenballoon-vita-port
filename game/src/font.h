...
#ifdef NATIVE_PORT
    /* FontData is overlaid directly on ASSET_FONTS (see load_fonts:
     * gFonts = fontAssetData + 1). Real 8-byte pointers here would inflate the
     * struct on LP64, shifting letter[] off 0x100 and breaking the gFonts[]
     * stride. These are runtime slots (filled by load_font with load_texture()
     * results, which are arena pointers), so a 4-byte dkrptr32 token keeps the
     * on-disk 0x400 layout and round-trips via DKR_PTR/DKR_TOK. */
/* 0x080 */ dkrptr32 texturePointers[32];
#else
...
#ifdef NATIVE_PORT
DKR_ASSERT_OFFSET(FontData, texturePointers, 0x80);
DKR_ASSERT_OFFSET(FontData, letter, 0x100);
DKR_ASSERT_SIZE(FontData, 0x400);
#endif
