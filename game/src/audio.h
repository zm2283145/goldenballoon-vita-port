...
#ifdef NATIVE_PORT
typedef struct MdkrAudioVoiceStats {
    u32 physicalAllocated;
    u32 physicalFree;
    u32 physicalLame;
    u32 musicAllocated;
    u32 musicFree;
...
void mdkr_audio_heap_stats(u32 *usedBytes, u32 *capacityBytes, u32 *allocations);
void mdkr_audio_voice_stats(MdkrAudioVoiceStats *stats);
/* Live-ownership high-water marks since the last read, sampled once per audio
 * pump. See the comment on mdkr_audio_voice_peaks_sample() in audio.c: the
 * instantaneous counts above are taken at a level boundary, where the clean-room
 * alCSPStop() has already released every music voice, so they cannot show that a
 * generation had live music ownership. These can. */
