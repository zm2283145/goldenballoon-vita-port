...
UNUSED u8 *screenimage_load(s32 screenIndex) {
    ...
    if (screenTableCount == 0) {
        ...
        someAddr = mempool_alloc_safe(size, COLOUR_TAG_BLUE);

        asset_load(ASSET_SCREENS, (uintptr_t) someAddr, start, size);
        mempool_free(screenTable);
...
UNUSED void screenimage_draw(Gfx **dList, u8 *screenAddress) {
    ...
    gDkrDmaDisplayList((*dList)++, OS_K0_TO_PHYSICAL(gRdpSetModeScreenAsset),
