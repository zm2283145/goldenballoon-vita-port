...
static f32 *racer_sound_prev_distance_slot(VehicleSoundData *soundData, s32 playerIndex) {
    ...
    if (playerIndex < (s32) ARRAY_COUNT(soundData->prevDistance)) {
        return &soundData->prevDistance[playerIndex];
    }
#ifdef NATIVE_PORT
    playerIndex -= ARRAY_COUNT(soundData->prevDistance);
    if (playerIndex < (s32) ARRAY_COUNT(soundData->prevDistanceExtra)) {
        return &soundData->prevDistanceExtra[playerIndex];
    }
#endif
    return NULL;
}
...
VehicleSoundData *racer_sound_init(s32 characterId, s32 vehicleId) {
    ...
    if (!mdkr_vehicle_sound_row(characterId, vehicleId, &assetRow, &soundVehicleId)) {
        stubbed_printf("racer_sound_init: invalid character/vehicle %d/%d\n", characterId, vehicleId);
        return NULL;
    }
    ...
    if (!mdkr_asset_rows_fit(table[ASSET_AUDIO_7], table[ASSET_AUDIO_8],
                             MDKR_VEHICLE_SOUND_ROW_COUNT, sizeof(VehicleSoundAsset))) {
        stubbed_printf("racer_sound_init: vehicle sound table is truncated\n");
        mempool_free(table);
        return NULL;
    }
...
void racer_sound_hovercraft(Object *obj, UNUSED u32 buttonsPressed, u32 buttonsHeld, s32 tickDelta) {
    ...
    pitchLevel = mdkr_mips_f64_to_u16((f64) gRacerSound->basePitch[0] * 10000.0);
...
void racer_sound_doppler_effect(Object *observerObj, Camera *camera, Object *sourceObj, s32 tickDelta) {
    ...
    */
    ...
    if (gRacerSound != NULL) {
        prevDistance = racer_sound_prev_distance_slot(gRacerSound, observerRacer->playerIndex);
        if (prevDistance == NULL || tickDelta == 0) {
            return;
        }
        if (camera != NULL) {
        ...
        relativeVelocity = (*prevDistance - distance) / tickDelta;
        ...
        /*
         * Some boss/cutscene racers intentionally have no active engine pitch.
         * log(0) is -inf, and converting that to s32 is undefined in C. There is
         * no meaningful Doppler shift without a positive finite source pitch.
         * The upper bound is far above the engine's authored 6.5534 cap and
         * also rejects infinity without depending on host libm predicates.
         */
        if (!(gRacerSound->basePitch[0] > 0.0f &&
              gRacerSound->basePitch[0] < 100.0f)) {
        ...
        cents = mdkr_mips_trunc_w_d(
            (f64) VEHICLE_LOG(gRacerSound->basePitch[0]) * 1731.23404); // 1200 / ln(2)
