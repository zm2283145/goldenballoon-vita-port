...
#ifdef NATIVE_PORT
/*
 * HudElement deliberately mirrors ObjectTransform's values, but it is not an
 * Object/ObjectSegment and may have weaker alignment. Copy the semantic fields
 * instead of type-punning it through the renderer ABI.
 */
static ObjectTransform mdkr_hud_element_transform(const HudElement *hud) {
    ObjectTransform transform;

    transform.rotation = hud->rotation;
    transform.spriteID = hud->spriteID;
    transform.scale = hud->scale;
    transform.position = hud->pos;
    return transform;
}
#endif

static s32 mdkr_hud_lap_index_valid(const Object_Racer *racer) {
    return racer->lap >= 0 && racer->lap < ARRAY_COUNT(racer->lap_times);
}

static s32 mdkr_hud_current_lap_time(const Object_Racer *racer, s32 fallback) {
    if (!mdkr_hud_lap_index_valid(racer)) {
        return fallback;
    }
    return racer->lap_times[racer->lap];
}
...
#ifdef NATIVE_PORT
    /* Split-screen evidence: gHUDNumPlayers IS the viewport layout (0-indexed:
     * ONE_PLAYER=0, TWO_PLAYERS=1, ...), and gNumActivePlayers is the resulting
     * viewport count. A two-player race must show hudPlayers=1 numViewports=2;
     * with only one wired pad it shows 0/1. Trace-only. */
    {
        extern int g_frameCounter;
        if (mdkr_trace_enabled()) {
            mdkr_trace("hud_init: hudPlayers=%d numViewports=%d @frame~%d", (int) gHUDNumPlayers,
                       (int) gNumActivePlayers, g_frameCounter);
        }
    }
#endif
...
#ifdef NATIVE_PORT
    /* LP64: the stale-counter byte array lives immediately after the entry[]
     * pointer array. The original steps past it as (s32 *), i.e. count*4 bytes,
     * but entry[] holds real void* (8 bytes each on a 64-bit host), so the counter
     * array overlapped the second half of entry[] and incrementing counters
     * corrupted the HUD element pointers (later free'd -> wild sprite_free). Step
     * past the real pointer array width. */
    gAssetHudElementStaleCounter = (u8 *) &gAssetHudElements->entry[gAssetHudElementIdsCount];
#else
...
#ifdef NATIVE_PORT
/**
 * Fidelity Phase 2b (census P7): the authoritative half of hud_render_player,
 * for ONE viewport. Everything here either integrates with the update rate,
 * consumes an input edge, or fires a sound/music cue -- none of which belongs to
 * the draw.
 *
 *   - gMinimapOpacity (and the gMinimapFade one-shot that simulation code arms
 *     through minimap_fade()) runs at the RAW rate and keeps running while
 *     paused, per the census's rate-group rules;
 *   - the C-button blocks CONSUME AN INPUT EDGE inside render, which is the
 *     spec 8 violation the census called out. One of them writes
 *     gHudToggleSettings, which render_scene itself re-reads later in the same
 *     frame through hud_setting() to decide the three-player TT viewport -- so
 *     the tick has to run BEFORE render_scene, which is where hud_tick() is
 *     called from;
 *   - gRaceStartShowHudStep drives music_play(SEQUENCE_RACE_START_FANFARE) and
 *     level_music_start();
 *   - the HUD slide/bounce state machine (gHudSlide, gHudBounceTimer/Mag/X,
 ...
 */
static void hud_player_tick(Object *obj, s32 updateRate) {
    Object_Racer *racer;
    ...
    if (obj == NULL || (gHudLevelHeader->unkBC & 2) || get_game_mode() == GAMEMODE_MENU) {
        return;
    }
    if (gMinimapFade) {
        gMinimapOpacity += updateRate;
...
/**
 * Fidelity Phase 2b (census P7): drive hud_player_tick over the frame's
 * viewports, in render_scene's own loop order and with render_scene's own
 * camera selection -- including the single-player PLAYER_TWO promotion, because
 * hud_render_player reads its viewport identity out of get_current_viewport()
 * (which is just gActiveCameraID) rather than off an argument.
 *
 * Must be called BEFORE render_scene: render_scene re-reads hud_setting() (i.e.
 * gHudToggleSettings, written here) later in the same frame to decide the
 * three-player TT viewport.
 */
void hud_tick(s32 updateRate) {
    ...
    /* Every other tick that shuffles gActiveCameraID puts it back
     * (obj_visibility_tick, scene_visibility_prepare_viewport's callers,
     * scene_build_last_viewport_basis); this one did not, so it left the active
     * camera on whichever viewport the HUD loop finished with. That is a
     * simulation-visible global -- get_current_viewport() reads it, and
     * set_temp_model_transforms keys its per-viewport LOD asset on it -- so
     * leaving it moved makes the tick's own exit state depend on player count.
     * The census's "gActiveCameraID shuffles (restored)" leave-alone rule
     * applies to a tick exactly as it applies to render. */
    savedCameraID = get_current_viewport();
    numViewports = scene_visibility_viewport_count();
    for (j = 0; j < numViewports; j++) {
        viewport = j;
        if (j == 0 && is_player_two_in_control() && numViewports == 1) {
            viewport = PLAYER_TWO;
        }
        set_active_camera(viewport);
        hud_player_tick(get_racer_object_by_port(viewport), updateRate);
    }
...
void hud_main_time_trial(s32 arg0, Object *playerRacerObj, s32 updateRate) {
    ...
    if (gStopwatchAnimID != 0xFF) {
        if (gStopwatchAnimID == 4 && (get_race_countdown() || !music_is_playing())) {
            ...
            temp_t6 = (DKR_PTR(ObjectModel_44, modInst->objModel->animations)[gStopwatchAnimID].animLength - 1) << 4;
            ...
            if (D_80126D69 < 0) {
                ...
                ttSWBodyObject->animFrame += (updateRate * D_80126D69);
                temp_lo = (DKR_PTR(ObjectModel_44, modInst->objModel->animations)[gStopwatchAnimID].animLength - 1) << 4;
    ...
    if (curRacer->raceFinished == FALSE) {
        ...
        for (i = 0; i < curRacer->countLap && i < gHudLevelHeader->laps &&
                    i < ARRAY_COUNT(curRacer->lap_times);
             i++) {
    ...
    if (is_in_time_trial()) {
        ...
        if (hud20 != NULL) {
            ...
            for (i = 0; i < numberOfBatches; i++) {
                if (D_80126D66 == DKR_PTR(TriangleBatchInfo, modInst->objModel->batches)[i].textureIndex) {
                    DKR_PTR(TriangleBatchInfo, modInst->objModel->batches)[i].textureIndex = arg0;
                    if (arg0 == 4) {
                        DKR_PTR(TriangleBatchInfo, modInst->objModel->batches)[i].flags |= (RENDER_UNK_0800000 | RENDER_TEX_ANIM);
                    ...
                    numberOfBatches = modInst->objModel->numberOfBatches;
                } else if (D_80126D65 == DKR_PTR(TriangleBatchInfo, modInst->objModel->batches)[i].textureIndex) {
                    DKR_PTR(TriangleBatchInfo, modInst->objModel->batches)[i].textureIndex = arg1;
                    numberOfBatches = modInst->objModel->numberOfBatches;
                }
            }
            D_80126D66 = arg0;
...
void hud_bananas(Object_Racer *racer, s32 updateRate) {
    ...
    if (racer->playerIndex == PLAYER_COMPUTER ||
        (((gHUDNumPlayers < TWO_PLAYERS || D_800E2794[gHUDNumPlayers][racer->playerIndex] == 2 ||
         ...
         (gHUDNumPlayers < TWO_PLAYERS || racer->lap < 1 || mdkr_hud_current_lap_time(racer, 180) >= 180 ||
...
void hud_race_position(Object_Racer *racer, s32 updateRate) {
    ...
    scale *= 128.0f;
    scale = sins_f(dkr_f32_to_s16_wrap(scale));
    placeIndex = place - 1;
...
void hud_lap_count(Object_Racer *racer, s32 updateRate) {
    if (racer->raceFinished == FALSE &&
        (gHUDNumPlayers <= ONE_PLAYER || racer->lap <= 0 || mdkr_hud_current_lap_time(racer, 180) >= 180) &&
    ...
    if (is_game_paused() == FALSE) {
        if (mdkr_hud_current_lap_time(racer, 30) < 30 &&
        ...
        if (gCurrentHud->entry[HUD_LAP_TEXT_LAP].lapText.status != LAPTEXT_IDLE) {
            if (gCurrentHud->entry[HUD_LAP_TEXT_LAP].lapText.status == LAPTEXT_UNK2) {
                if (gCurrentHud->entry[HUD_LAP_TEXT_LAP].lapText.direction == LAPTEXT_OUT) {
                    ...
                    if (mdkr_hud_current_lap_time(racer, 60) >= 60) {
                        gCurrentHud->entry[HUD_LAP_TEXT_LAP].lapText.direction = LAPTEXT_IN;
                        sound_play(SOUND_WHOOSH1, NULL);
                    }
                    if (gCurrentHud->entry[HUD_LAP_TEXT_LAP].pos.x ==
    ...
    switch (gCurrentHud->entry[HUD_CHALLENGE_FINISH_POS_1].raceFinishPos.status) {
        case 0:
            ...
            if (gNumActivePlayers != 1) {
                ...
                temp_v0_4 = 0;
                for (updateRate = 0;
                     temp_v0_4 < curLevelHeader->laps &&
                     updateRate < ARRAY_COUNT(racer->lap_times);
                     updateRate++, temp_v0_4++) {
        ...
        if (racer->balloon_quantity > 0) {
            ...
            if (gCurrentHud->entry[HUD_WEAPON_DISPLAY].weaponDisplay.rotation > 16) {
                ...
                if (gCurrentHud->entry[HUD_WEAPON_DISPLAY].weaponDisplay.scale > 120) {
                    ...
                    gCurrentHud->entry[HUD_WEAPON_DISPLAY].scale +=
                        0.18 *
                        sins_f(dkr_f32_to_s16_wrap(
                            (f32)gCurrentHud->entry[HUD_WEAPON_DISPLAY].weaponDisplay.scale * 682.6583f * 4.0f));
                }
            }
            if (gHUDNumPlayers > ONE_PLAYER) {
...
#ifdef NATIVE_PORT
        {
            ObjectTransform transform = mdkr_hud_element_transform(hud);
            render_sprite_billboard_transform(
                &gHudDL, &gHudMtx, &gHudVtx, &transform, hud->spriteOffset,
                entry, RENDER_Z_UPDATE);
        }
#else
...
void hud_race_time(Object_Racer *racer, s32 updateRate) {
    ...
    if (!(gHUDNumPlayers != ONE_PLAYER && D_800E2794[gHUDNumPlayers][racer->playerIndex] != 1) ||
        (gHUDNumPlayers > ONE_PLAYER && racer->lap > 0 &&
         mdkr_hud_current_lap_time(racer, 180) < 180)) {
        if (racer->raceFinished == FALSE) {
            timerHideCounter = gCurrentHud->entry[HUD_RACE_TIME_LABEL].raceTimeLabel.hideTimer + 127;
            if (racer->lap > 0 && mdkr_hud_lap_index_valid(racer) &&
                racer->lap_times[racer->lap] < 180 && racer->lap < gHudLevelHeader->laps) {
                ...
                stopwatchTimer = 0;
                for (i = 0; i <= racer->countLap && i < gHudLevelHeader->laps &&
                            i < ARRAY_COUNT(racer->lap_times);
                     i++) {
...
void minimap_init(LevelModel *model) {
    ...
    model->minimapSpriteIndex =
        (s32) DKR_TOK(tex_load_sprite(model->minimapSpriteIndex, 1));
}
...
#ifdef NATIVE_PORT
        if (mdkr_trace_enabled()) {
            static s32 reportedMirroredMinimap;
            if (!reportedMirroredMinimap) {
                mdkr_trace("adventure_hud: minimapYaw=%d minimapX=%.1f",
                           (int) hudElem.rotation.y_rotation, hudElem.pos.x);
                reportedMirroredMinimap = TRUE;
            }
        }
#endif
    } else {
...
#ifdef NATIVE_PORT
    /*
     * The native transform adapter copies this field even though the minimap
     * call supplies its Sprite explicitly. The matching path never reads it;
     * initialize it here so optimized GCC cannot observe indeterminate data.
     */
    hudElem.spriteID = 0;
#endif
    hudElem.pos.z = 0.0f;
...
#ifdef NATIVE_PORT
    {
        ObjectTransform transform = mdkr_hud_element_transform(&hudElem);
        render_ortho_triangle_image_transform(
            &gHudDL, &gHudMtx, &gHudVtx, &transform, hudElem.spriteOffset,
            minimap, 0);
    }
#else
    render_ortho_triangle_image(
        &gHudDL, &gHudMtx, &gHudVtx, (ObjectSegment *) &hudElem, minimap, 0);
#endif
...
#ifdef NATIVE_PORT
        ObjectTransform transform = mdkr_hud_element_transform(hud);
        mtx_cam_push(&gHudDL, &gHudMtx, &transform, 1.0f, 0.0f);
#else
...
void hud_draw_model(ObjectModel *objModel) {
    ...
    for (i = 0; i < numBatches; i++) {
        flags = DKR_PTR(TriangleBatchInfo, objModel->batches)[i].flags;
        if (!(flags & RENDER_Z_UPDATE)) {
            ...
            numVerts = DKR_PTR(TriangleBatchInfo, objModel->batches)[i + 1].verticesOffset - vertOffset;
            numTris = DKR_PTR(TriangleBatchInfo, objModel->batches)[i + 1].facesOffset - triOffset;
            verts = &DKR_PTR(Vertex, objModel->vertices)[vertOffset];
            tris = &DKR_PTR(Triangle, objModel->triangles)[triOffset];
            texIndex = DKR_PTR(TriangleBatchInfo, objModel->batches)[i].textureIndex; // 0xFF = No Texture
            textureEnabled = TRIN_ENABLE_TEXTURE;
            if (texIndex == 0xFF) {
                texPtr = NULL;
            } else {
                texPtr = DKR_PTR(TextureHeader, DKR_PTR(TextureInfo, objModel->textures)[texIndex].texture);
            }
