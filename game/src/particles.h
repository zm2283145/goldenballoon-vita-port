...
#ifdef NATIVE_PORT
    /* 0x9C */ dkrptr32 colourLoop;
    /* NATIVE_PORT: must stay a 4-byte slot. On disk this word is either -1 ("no
     * colour loop") or a misc-asset index that init_particle_assets() replaces in
     * place with the resolved arena pointer. A real 8-byte pointer here would be
     * pushed to 0xA0 by alignment and grow sizeof(ParticleBehaviour) to 0xA8,
     * while the on-disk record stride is 0xA0 (asset_swap.c
     * PARTICLE_BEHAVIOUR_SIZE) — so every read of colourLoop picks up the next
     * record's velocityModifier floats instead. Read it via PARTICLE_COLOUR_LOOP.
     */
#else
...
#ifdef NATIVE_PORT
_Static_assert(sizeof(ParticleBehaviour) == 0xA0,
               "ParticleBehaviour must keep the 0xA0 on-disk record stride");
_Static_assert(__builtin_offsetof(ParticleBehaviour, colourLoop) == 0x9C,
               "ParticleBehaviour.colourLoop must stay at 0x9C");
/* Resolve the 4-byte colourLoop slot to what the N64 code expects in a
 * ColorLoopEntry*: the (ColorLoopEntry *)-1 sentinel, or the arena pointer whose
 * low 32 bits the slot holds. Callers test it with `(s32)x != -1` / `(u32)x != -1U`,
 * both of which stay correct for a 64-bit all-ones pointer. */
static inline ColorLoopEntry *mdkr_particle_colour_loop(const ParticleBehaviour *b) {
    if (b->colourLoop == 0xFFFFFFFFu) {
        return (ColorLoopEntry *) (intptr_t) -1;
    }
    return DKR_PTR(ColorLoopEntry, b->colourLoop);
}
#define PARTICLE_COLOUR_LOOP(b) mdkr_particle_colour_loop(b)
#else
#define PARTICLE_COLOUR_LOOP(b) ((b)->colourLoop)
#endif
