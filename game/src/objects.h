...
#ifdef NATIVE_PORT
/* Byte length of an ASSET_MISC sub-asset (0 if the index is out of range).
 * Needed by the on-demand asset_swap_misc_* converters, which must bound their
 * walk to the blob: gAssetsMiscTable entries are s32-WORD offsets, so the byte
 * length is (table[index + 1] - table[index]) * 4. */
s32 get_misc_asset_size(s32 index);
#endif
#ifdef NATIVE_PORT
/* Byte-swap an ASSET_MISC sub-asset that is a 32-bit-word (f32 or s32) array, in
 * place and once. Call before reading such a sub-asset as a word pointer. */
void dkr_misc_swap_words(s32 index);

/* The 16-bit sibling, for sub-assets read only as s16[]/u16[]. NOT interchangeable
 * with dkr_misc_swap_words: a 32-bit swap also transposes the two halfwords within
 * each word. Shares the one-shot dedup. */
void dkr_misc_swap_halfwords(s32 index);

/* Normalize every known ASSET_MISC sub-asset, once, at section load: the 32-bit
 * word arrays, the 16-bit halfword arrays, and the per-field record swizzles, then
 * verify the results are plausible. These are the authoritative lists — prefer
 * adding an index there over a new lazy call site. See the block comment on the
 * definition in objects.c. */
void dkr_misc_normalize_tables(void);

/* The boost / exhaust graphics table (ASSET_MISC_20). The raw sub-asset is
 * big-endian AND has a 0x80 on-disk stride against sizeof(Object_Boost)==0x88 on
 * LP64, so it must NOT be indexed in place — dkr_boost_table() returns a
 * converted, native-layout array instead. See the block comment on the
 * definition in objects.c. Every ASSET_MISC_20 consumer goes through the macro
 * below so nothing can accidentally index the raw blob. */
Object_Boost *dkr_boost_table(void);
#define GET_BOOST_TABLE() dkr_boost_table()

/* Headless verification hook for the boost graphics: MDKR_FORCE_BOOST=<frame>[:<len>]
