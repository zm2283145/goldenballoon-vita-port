...
void obj_loop_trophycab(Object *obj, s32 updateRate) {
        ...
        if (header->race_type != RACETYPE_CUTSCENE_2 && header->race_type != RACETYPE_CUTSCENE_1) {
            ...
            if (mdkr_trophy_state(settings->trophies, settings->worldId,
                ...
                if (mdkr_trace_enabled()) {
                    mdkr_trace("trophycabinet: DISPLAY world=%d state=%u "
                               "trophies=0x%x @frame~%d",
                               (int) settings->worldId, (unsigned) trophyState,
                               (unsigned) settings->trophies, g_frameCounter);
                }
#endif
    ...
    if (tempObj != NULL) {
#ifdef NATIVE_PORT
        mdkr_trophy_control_collision(obj, tempObj);
#endif
...
#ifdef NATIVE_PORT
                    if (mdkr_trace_enabled()) {
                        mdkr_trace("trophycabinet: ENTER world=%d course=%d "
...
/*
 * Production Fire Mountain terminal predicate. Keeping this as a named seam
 * lets the native challenge fixture deliver a completed hatch without assigning
 * a verdict itself; ordinary gameplay reaches it from EGG_HATCHED below.
 */
void racer_challenge_egg_finish_if_complete(Object_Racer *racer) {
    if (racer == NULL || racer->lap < 3) {
        return;
    }
#ifdef NATIVE_PORT
    if (!mdkr_challenge_terminal_gate_enabled(RACETYPE_CHALLENGE_EGGS)) {
        return;
    }
#endif
    ...
    switch (egg->status) {
        ...
        case EGG_IN_BASE:
            ...
            if (racerObj != NULL && egg->status != EGG_IN_BASE) {
                racerObj->racer->eggHudCounter -= 1;
            }
            break;
        case EGG_HATCHED:
            racerObj = get_racer_object(egg->racerID);
            if (racerObj != NULL) {
                racer = racerObj->racer;
                racer_challenge_egg_finish_if_complete(racer);
            }
            obj->animFrame = 128;
            break;
    }
...
void obj_init_animator(Object *obj, LevelObjectEntry_Animator *entry, s32 param) {
    ...
    if (animator->segmentId != -1) {
        ...
        segmentBatchCount = DKR_PTR(LevelModelSegment, levelModel->segments)[animator->segmentId].numberOfBatches;
...
void obj_loop_animator(Object *obj, s32 updateRate) {
    ...
    levelModel = get_current_level_model();
    curBlock = &DKR_PTR(LevelModelSegment, levelModel->segments)[animator->segmentId];
    curBatch = &DKR_PTR(TriangleBatchInfo, curBlock->batches)[animator->batchId];
    ...
    if (textureIndex != TEX_INDEX_NO_TEXTURE) {
        if (!curBatch) {} // Fake
        tex = DKR_PTR(TextureHeader, DKR_PTR(TextureInfo, levelModel->textures)[textureIndex].texture);
        shift2 = tex->width << 7;
        shift = tex->width << 7;
        for (tri = trisStart; tri < trisEnd; tri++) {
            curTriangle = &DKR_PTR(Triangle, curBlock->triangles)[tri];
                ...
                for (i = 0; i < wizShipObj->attachPoints->count; i++) {
                    ...
                    if ((index >= 0) && (index < wizShipModel->numberOfAttachPoints)) {
                        if (wizShipObj->curVertData != NULL) {
                            posX = wizShipObj->curVertData[DKR_PTR(s16, wizShipModel->attachPoints)[index]].x;
                            posY = wizShipObj->curVertData[DKR_PTR(s16, wizShipModel->attachPoints)[index]].y;
                            posZ = wizShipObj->curVertData[DKR_PTR(s16, wizShipModel->attachPoints)[index]].z;
...
#ifdef NATIVE_PORT
/* Lock the three things obj_loop_snowball() below relies on when it hands
 * `&animated->soundMask` to audspat_play_sound_at_position() as an `AudioPoint **`.
 * audspat_point_create() stores a whole host pointer through that address, and
 * AudioPoint::userHandlePtr retains it to null out later, so the slot must be
 * pointer-wide, pointer-aligned, and must not overlap the field that follows it.
 * As the decomp's `u32` all three failed at once: the readback truncated to 32 bits
 * (the level-9 crash) and the top half of the pointer landed on `currentSound`.
 * Reverting the field to an integer, or reordering the struct so `currentSound`
 * moves back inside the store's footprint, must not compile. */
_Static_assert(sizeof(((Object_AnimatedObject *) 0)->soundMask) == sizeof(AudioPoint *),
               "Object_AnimatedObject.soundMask must hold a whole host AudioPoint *");
_Static_assert(offsetof(Object_AnimatedObject, soundMask) % sizeof(AudioPoint *) == 0,
               "Object_AnimatedObject.soundMask must be pointer-aligned for the AudioPoint ** store");
_Static_assert(offsetof(Object_AnimatedObject, currentSound) >=
                   offsetof(Object_AnimatedObject, soundMask) + sizeof(AudioPoint *),
               "Object_AnimatedObject.currentSound overlaps the AudioPoint ** store into soundMask");
#endif
        ...
        if (modInst != NULL) {
            ...
            if (playerIndex) {
                ...
                if (numCursors > 0) {
                    ...
                    for (i2 = 0; i2 < objMdl->numberOfBatches; i2++) {
                        ...
                        // can't be less.
                        if (DKR_PTR(TriangleBatchInfo, objMdl->batches)[i2].textureIndex >= 0 && DKR_PTR(TriangleBatchInfo, objMdl->batches)[i2].textureIndex < 4) {
            ...
            if (charSelectObj->animationID >= 0) {
                if ((charSelectObj->animationID < objMdl->numberOfAnimations)) {
                    i = (DKR_PTR(ObjectModel_44, objMdl->animations)[charSelectObj->animationID].animLength - 1) << 4;
...
void obj_init_goldenballoon(Object *obj, LevelObjectEntry_GoldenBalloon *entry) {
    ...
    if (entry->challengeID >= 1 && entry->challengeID <= 3) {
        if (get_settings()->tajFlags &
            (UINT32_C(1) << (entry->challengeID + 2))) {
            obj->properties.goldenBalloon.action = 0;
        } else {
...
void obj_loop_goldenballoon(Object *obj, s32 updateRate) {
    ...
    if (!mdkr_course_flag(levelEntry->goldenBalloon.balloonID, &flag)) {
        free_object(obj);
        return;
    }
...
void obj_loop_door(Object *doorObj, s32 updateRate) {
    ...
    door = doorObj->door;
    if (mdkr_course_flag(door->doorID, &doorIDFlag)) {
        ...
        if (playSound) {
            ...
            if (door->soundMask != NULL) {
                audspat_point_stop((AudioPoint *) (uintptr_t) door->soundMask);
                door->soundMask = NULL;
            }
        }
...
void obj_loop_ttdoor(Object *obj, s32 updateRate) {
    ...
    ttDoor = obj->door;
    settings = get_settings();
    amulet = settings->ttAmulet;
    if (ttDoor->doorType == 0) {
        requestedModel =
            (amulet >= 0 && amulet < ARRAY_COUNT(D_800DCA94))
                ? D_800DCA94[amulet]
                : 0;
    } else {
        requestedModel =
            (amulet >= 0 && amulet < ARRAY_COUNT(D_800DCA9C))
                ? D_800DCA9C[amulet]
                : 0;
    }
    if (!mdkr_model_index_resolve(
            requestedModel, obj->header->numberOfModelIds,
            &requestedModel)) {
        requestedModel = 0;
    }
    obj->modelIndex = requestedModel;
    ...
    if (openDoor) {
        ...
        audspat_point_stop((AudioPoint *) ((uintptr_t) ttDoor->soundMask));
...
void obj_loop_trigger(Object *obj, UNUSED s32 updateRate) {
    ...
    if (mdkr_course_flag(triggerEntry->index, &flags)) {
        if (obj->interactObj->distance < trigger->radius) {
...
/*
 * Production Smokey Castle deposit transition. The terminal-event fixture calls
 * the same function as a real arriving flycoin, so lap/score and the finish
 * predicate cannot drift apart.
 */
void racer_challenge_banana_deposit(Object_Racer *racer) {
    if (racer == NULL) {
        return;
    }
    racer->lap++;
...
#ifdef NATIVE_PORT
    if (!mdkr_challenge_terminal_gate_enabled(RACETYPE_CHALLENGE_BANANAS)) {
        return;
    }
#endif
    racer->raceFinished = TRUE;
...
void obj_loop_flycoin(Object *obj, s32 updateRate) {
    ...
    if (obj->properties.flyCoin.diff <= 0) {
        ...
        racer_challenge_banana_deposit(racerObj);
        free_object(obj);
...
void obj_init_weaponballoon(Object *obj, LevelObjectEntry_WeaponBalloon *entry) {
    ...
    if (obj->header->numberOfModelIds <= 0) {
        free_object(obj);
        return;
    }
    if (obj->modelIndex < 0 || obj->modelIndex >= obj->header->numberOfModelIds) {
        obj->modelIndex = 0;
    }
...
void obj_loop_texscroll(Object *obj, s32 updateRate) {
    ...
    tex = DKR_PTR(TextureHeader, DKR_PTR(TextureInfo, levelModel->textures)[texScroll->textureIndex].texture);
    ...
    curBlock = DKR_PTR(LevelModelSegment, levelModel->segments);
...
void obj_init_midifade(Object *obj, LevelObjectEntry_MidiFade *entry) {
    ...
    vertex = &DKR_PTR(Vertex, objModel->vertices)[1];
    maxX = vertex->x;
    maxY = vertex->y;
    maxZ = vertex->z;
    minX = maxX;
    ...
    for (i = 1; i < objModel->numberOfVertices; i++) {
        vertex = &DKR_PTR(Vertex, objModel->vertices)[i];
        if (vertex->x < minX) {
            minX = vertex->x;
        }
        if (maxX < vertex->x) {
...
void obj_init_boost(Object *obj, LevelObjectEntry_Boost2 *entry) {
    Object_Boost *asset20 = GET_BOOST_TABLE();
#ifdef NATIVE_PORT
    size_t entrySize = entry->common.size & 0x3F;
    s32 racerIndex = 0;

    /*
     * Retail object maps contain ordinary eight-byte BHV_BOOST records, while
     * racerfx_alloc() creates ten-byte records with a private racerIndex tail.
     * The original initializer unconditionally read byte eight. For a map
     * record that is the first byte of the NEXT record (measured as 0x94 in the
     * title sequence), producing an out-of-bounds boost-table pointer even
     * though the dormant map object normally never dereferences it.
     */
    if (entrySize >= sizeof(LevelObjectEntry_Boost2)) {
        racerIndex = entry->racerIndex;
    }
    if ((u32) racerIndex >= NUMBER_OF_CHARACTERS) {
        fprintf(stderr,
                "[FATAL] boost entry has racer index %d (valid range 0..%d)\n",
                racerIndex, NUMBER_OF_CHARACTERS - 1);
        abort();
    }
    obj->boost = &asset20[racerIndex];
#else
...
#ifdef NATIVE_PORT
    f32 colY[COLLISION_Y_QUERY_CAPACITY];
#else
    f32 colY[8];
#endif
    UNUSED s32 pad2[4];
...
#ifdef NATIVE_PORT
            if (collision_get_y(obj->segmentID, obj->trans.x_position, obj->trans.z_position, colY,
                                ARRAY_COUNT(colY)) != 0) {
#else
