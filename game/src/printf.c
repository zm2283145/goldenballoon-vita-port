...
#define is_longlong 0
        ...
        switch (fc) {
            ...
            case 'E': {
                ...
                /*
                 * Every floating argument is promoted to double in a variadic
                 * call, including the custom short-format spelling. Reading a
                 * float here is undefined on every native ABI.
                 */
                nextarg(spD0, f64);
            ...
            case 'p':
                /* Generic pointer.  */
                {
                    ...
                    if (ptr != NULL) {
                        ...
                        fc = 'x';
                        alt = 1;
                        /* uintptr_t, not `unsigned long`: Windows is LLP64, so
                         * `long` is 32 bits there and this cast silently
                         * truncated the top half of every %p. (Caught by
                         * -Werror=pointer-to-int-cast under mingw-w64.) */
                        num = (uintptr_t) ptr;
                        is_neg = 0;
                        goto number;
                    } else {
