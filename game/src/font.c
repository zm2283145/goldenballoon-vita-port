...
#if REGION != REGION_JP
#ifdef NATIVE_PORT
static size_t font_atlas_regions(const FontData *fontData, s32 textureIndex,
                                 GfxFontAtlasRegion regions[96]) {
    size_t count = 0;

    if (fontData == NULL || regions == NULL || textureIndex < 0) {
        return 0;
    }
    for (s32 character = 0; character < 96; character++) {
        const FontCharData *glyph = &fontData->letter[character];
    ...
    if (fontID < gNumberOfFonts) {
        ...
        if (fontData->loadedFonts[0] == 1) {
            ...
            while (i < 32 && fontData->textureID[i] != -1) {
                TextureHeader *texture = load_texture(fontData->textureID[i]);
#ifdef NATIVE_PORT
                if (texture == NULL) {
                    fprintf(stderr,
                            "[FATAL] font %d texture slot %d (asset %d) "
                            "failed to load\n",
                            fontID, i, fontData->textureID[i]);
                    abort();
                }
#endif
                fontData->texturePointers[i] = DKR_TOK(texture);
#ifdef NATIVE_PORT
                if (texture != NULL) {
                    GfxFontAtlasRegion regions[96];
                    size_t regionCount =
            ...
            if (fontData->loadedFonts[0] == 0) {
                ...
                while (i < 32 && fontData->textureID[i] != -1) {
                    TextureHeader *texture =
                        DKR_PTR(TextureHeader, fontData->texturePointers[i]);
#ifdef NATIVE_PORT
                    if (texture != NULL) {
                        gfx_dkr_font_texture_unregister(texture + 1);
                    }
#endif
                    tex_free(texture);
                    fontData->texturePointers[i] = DKR_TOK(NULL);
                    i++;
                }
            }
        }
    ...
    if (font < gNumberOfFonts) {
        ...
        if (fontData->loadedFonts[0] != 0) {
            ...
            if (pointerIndex != 0xFF) {
                return DKR_PTR(TextureHeader, fontData->texturePointers[pointerIndex]);
            }
            return NULL;
        }
    }
            ...
            if (curChar <= 0x20 || curChar >= 0x80) {
                ...
                if (textureIndex != 0xFF) {
                    ...
                    if (lastTextureIndex != textureIndex) {
                        ...
                        texture = DKR_PTR(TextureHeader, fontData->texturePointers[textureIndex]);
                        gDkrDmaDisplayList((*dList)++, OS_K0_TOKEN_TO_PHYSICAL(texture->cmd),
...
void func_800C6464_C7064(void) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        for (charIndex = 0; charIndex < JP_FONT_ARRAY_SIZE; charIndex++) {
            asset_load(ASSET_JAPANESE_FONTS, (uintptr_t)jpFontData,
...
void fontCreateDisplayList(Gfx *dList, Asset46 *asset, s32 width, s32 height) {
    ...
    if (asset->unk0 != -1) {
        texture = (u8 *) asset + asset->unk0;
    } else {
        texture = NULL;
    }
    if (asset->unk4 != -1) {
        mask = (u8 *) asset + asset->unk4;
    } else {
        mask = NULL;
    }

    if (mask != NULL) {
        gDPLoadTextureBlockS(dList++, OS_K0_TO_PHYSICAL(texture), G_IM_FMT_RGBA, G_IM_SIZ_16b, width, height,
                             0,                         // palette
        ...
        gDPLoadMultiBlock_4bS(dList++, OS_K0_TO_PHYSICAL(mask), 0x100, 1, G_IM_FMT_I, width, height,
                              0,                         // palette
                              G_TX_NOMIRROR | G_TX_WRAP, // cms
                              G_TX_NOMIRROR | G_TX_WRAP, // cmt
                              8,                         // masks
        ...
        switch (asset->unk8) {
            ...
            case 6: // IA4
                ...
                break;
            case 3: // I4
        gDPLoadTextureBlock_4bS(dList++, OS_K0_TO_PHYSICAL(texture), G_IM_FMT_I, width, height,
                                        0,                         // palette
                                        G_TX_NOMIRROR | G_TX_WRAP, // cms
                                        G_TX_NOMIRROR | G_TX_WRAP, // cmt
                                        8,                         // masks
