...
#ifdef NATIVE_PORT
    /* NULL terminator for the DrawTexture[] scan in texrect_draw / texrect_draw_scaled
     * (`for (i=0; (tex=element[i].texture); i++)`). On LP64 DrawTexture is 16 bytes
     * (its texture pointer grew 4->8), so element[1] lands here and its 8-byte
     * texture field must read fully zero. A 4-byte s32 unk8 leaves the upper half as
     * uninitialised stack padding -> element[1].texture reads a wild pointer and the
     * scan runs off the single element. Pointer-width so `unk8 = 0` zeroes it all. */
    void *unk8;
#else
