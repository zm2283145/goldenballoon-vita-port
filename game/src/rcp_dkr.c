...
#ifdef NATIVE_PORT
    dkrtask->task.ucode_boot_size = 0;
#else
    dkrtask->task.ucode_boot_size =
...
UNUSED void gfxtask_run_fifo2(Gfx *dlBegin, Gfx *dlEnd, s32 recvMesg) {
    ...
    dkrtask = &gGfxTaskBuf[gGfxBufCounter];
    gGfxBufCounter++;
    if (gGfxBufCounter == ARRAY_COUNT(gGfxTaskBuf)) {
        gGfxBufCounter = 0;
    }
...
s32 gfxtask_wait(void) {
    ...
    osRecvMesg(&gGfxTaskMesgQueue, &mesg, OS_MESG_BLOCK);
    gGfxTaskIsRunning = FALSE;
    payload = (OSMesg *) mesg;
    return payload != NULL
#ifdef NATIVE_PORT
               ? mdkr_s32_from_opaque(payload[1])
#else
               ? (s32) (intptr_t) payload[1]
#endif
               : 0;
}
...
void bgdraw_texture(Gfx **dList) {
    ...
    if (gTexBGTex2 == NULL) {
        ...
        gDkrDmaDisplayList((*dList)++, OS_K0_TOKEN_TO_PHYSICAL(gTexBGTex1->cmd),
                           gTexBGTex1->numberOfCommands);
        videoWidth <<= 2;
        videoHeight <<= 2;
        ...
        gDkrDmaDisplayList((*dList)++, OS_K0_TOKEN_TO_PHYSICAL(gTexBGTex2->cmd),
                           gTexBGTex2->numberOfCommands);
...
void texrect_draw(Gfx **dList, DrawTexture *element, s32 xPos, s32 yPos, u8 red, u8 green, u8 blue, u8 alpha) {
    ...
    for (i = 0; (tex = element[i].texture); i++) {
        ulx = (element[i].xOffset * 4) + xPos;
        uly = (element[i].yOffset * 4) + yPos;
        lrx = ((s32)tex->width * 4) + ulx;
        lry = ((s32)tex->height * 4) + uly;
        if (lrx > 0 && lry > 0) {
            s = 0;
            t = 0;
            if (ulx < 0) {
