...
#ifdef NATIVE_PORT
        /*
         * A transition is a mask over the complete host output, not HUD art.
         * Use the renderer's aspect-preserving cover space so ultrawide side
         * pixels are never exposed and circle transitions stay circular.
         */
        mtx_ortho_fullscreen(dList, mtx);
#else
        mtx_ortho(dList, mtx);
#endif
