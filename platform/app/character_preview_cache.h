...
// Reserve one launcher-owned PNG location for this package/context. Any prior
// regular file at that exact derived location is removed; unrelated files,
// directories, and links are never touched. The renderer still creates the
// PNG exclusively, so this function does not weaken its no-overwrite rule.
...
// Choose the other member of a bounded two-slot product/context pair when
// retainedPath names the currently published member. This lets a recapture
// remain create-only without deleting the last good digest-bound image before
// ROM validation and rendering succeed. A nonempty foreign retainedPath fails.
...
// True only for one of the bounded exact paths derived from this package ID.
bool owns(const std::string &charactersDirectory,
          const std::string &packageId,
          const std::string &capturePath);

// Remove both exact custom-character slots for one package/context when they
// are regular, non-link files. Missing files are success. Suspicious
// filesystem entries are retained.
