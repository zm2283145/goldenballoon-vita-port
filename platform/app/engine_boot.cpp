...
int mdkr64_engine_boot(const MdkrBootConfig *cfg) {
    ...
    if (cfg != nullptr) {
        ...
        if ((cfg->character_preview_studio || cfg->character_motion_review ||
            ...
            cfg->character_preview_context == MDKR_CHARACTER_PREVIEW_NONE) {
            std::fprintf(stderr,
                         "[app] boot rejected character studio/review/reference without preview\n");
            return 2;
        }
        ...
        if (cfg->character_preview_donor_reference != 0 &&
            cfg->character_preview_donor_reference != 1) {
            std::fprintf(stderr,
                         "[app] boot rejected invalid donor-reference flag\n");
            return 2;
        }
    ...
    if (cfg != nullptr &&
        ...
        environmentReady = previewEnvironment.set(
            ...
            previewEnvironment.set(
                ...
                (pose != nullptr ||
                 cfg->character_preview_donor_reference) && captureRequested
                    ? captureKind : "") &&
            environmentReady;
        for (int player = 0; player < 4; ++player) {
            const std::string variable =
        ...
        std::fprintf(
            ...
            "view=%d,%d lighting=%s captureKind=%s autoReturn=%d motionReview=%d donorReference=%d "
            "capture=%s\n",
        ...
        if (cfg->character_preview_result != nullptr) {
            *cfg->character_preview_result = MdkrCharacterPreviewResult{};
            ...
            cfg->character_preview_result->capture_kind =
                cfg->character_preview_capture_kind;
            cfg->character_preview_result->donor_reference =
                cfg->character_preview_donor_reference;
