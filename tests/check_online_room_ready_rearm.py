...
its own verdict; this harness asserts every sub-flag so a regression names itself:
  ...
  totalFires=2      -- exactly one native takeover per tournament (no re-boot loop)
  ...
  clearedWhileFalse=1 -- the latch clears ONLY while out of the takeover condition
...
VERDICT_RE = re.compile(
    ...
    r"t2Once=(\d+) routed2=(\d+) verdict=(PASS|FAIL)$",
    re.MULTILINE)
...
def main() -> int:
    ...
    (total_fires, t1_once, left_no_rearm, finished_no_instant,
     cleared_while_false, t2_once, routed2, verdict) = match.groups()
    ...
    if verdict != "PASS" or rc != 0 or total_fires != "2" or bad:
    ...
    print(
        ...
        "re-took native EXACTLY ONCE for tournament #2 (route=lobby-start, "
        f"totalFires={total_fires})")
    return 0
